...
function setNewHeader(){
	...
	{
		ratingHtml = '<div class="rating"><div class="iblock-vote small"><table class="table-no-border"><tr>';
...
window.JCCatalogElement.prototype.SelectOfferProp = function()
{
	var i = 0,
		...
		RowItems = null,
		target = BX.proxy_context;
    $('.item_slider .slides .big_view180d').remove();
    $('.item_slider .slides .offers_img').show();
...
window.JCCatalogElement.prototype.ChangeInfo = function()
{
	...
	if (-1 < index)
	{
        ...
        if (this.offers[index].DISPLAY_PROPERTIES_CODE.ARTICLE.IS_3DVIEW == "Y"){
            ...
            $('.arrangement-block a').attr('href', '/3d-online/?artcode=' + enartikul);
            $(".arrangement-block").show();
		}else{
            $(".arrangement-block").hide();
            $('.arrangement-block a').attr('href', '');
		}
...
window.JCCatalogElement.prototype.SetSliderPict = function(obj, slider, config)
{
	...
	for(var i in slider){
		if (slider[i].VIEW180D == "Y"){
			delete slider[i];
			console.log('udalili lishnee!!!');
			continue;
		}
		if(typeof(slider[i]) == 'object' && !slider[i].VIEW180D){
			slideHtml+='<li data-slide_key="'+i+'" data-big="'+slider[i].BIG.src+'">'+
			'<span class="cnt"><img border="0" src="'+slider[i].THUMB.src+'" alt="'+slider[i].ALT+'" title="'+slider[i].TITLE+'" /></span>'+
			'</li>';
		}
	}
    console.log('dalee slider'); console.log(slider);
	if (obj.DISPLAY_PROPERTIES_CODE.ARTICLE.IS_3DVIEW == "Y"){
		console.log('Y');console.log(i);
		i++;
        slideHtml+='<li data-slide_key="'+i+'" class="tview180d">'+
            '<span class="view180d" id="view180d"></span>'+
            '</li>';
        slider[i] = {
        	VIEW180D: "Y",
            ARTICLE: encodeURIComponent(obj.DISPLAY_PROPERTIES_CODE.ARTICLE.VALUE),
				BIG: "",
            SMALL: "",
			THUMB: ""
		};
...
console.info(slideHtml);
	...
	if(countPhoto>1 || obj.POPUP_VIDEO){
		...
		container.find('.thumbs').delegate('li:not(.current)', 'click mouseenter', function(e){
			var srcSmall='',
				srcBig='',
				width='',
				height=''
				index=$(this).data('slide_key');
			console.info('mouse-test: ' + index);console.log(slider[index]);
			if(typeof slider[index] !== 'undefined')
			{
                ...
                if('VIEW180D' in slider[index]){
                    view180d = slider[index].VIEW180D;
                }else{
                	view180d = "";
				}
			}
console.log('view:' +  view180d);
...
console.info('proveryaem div!');
//$('.item_slider .slides .big_view180d').length == 0 &&
			if (view180d && slider[index]){
                console.info('dobavlyaem div!');
                $('.item_slider .slides .big_view180d').remove();
				if ($('.item_slider .slides .big_view180d').length == 0) {
                    console.info('dobavili div!');
                    $('.item_slider .slides').append('<div class="big_view180d" id="big_view180d"></div>');
                    $('.item_slider .slides .offers_img').hide();
                    var Viewer3d = new DView3dConstructor(document.getElementById('big_view180d'), 800, 534, 'https://o3d.ru/');
                    Viewer3d.Initialize('https://o3d.ru/defo/view3d_models.php?code=' + slider[index].ARTICLE);
                }
			}else{console.info('udalyaem div!');
			console.log(view180d);
			console.log($('.item_slider .slides .big_view180d').length);
			console.log($('.item_slider .slides .big_view180d'));
                $('.item_slider .slides .big_view180d').remove();

                console.info('udalili div!');
                $('.item_slider .slides .offers_img').show();
			}
		...
		if(typeof(obPrices) == 'object')
		{
			...
			for(var j in obPrices)
			{
				if(obPrices[j] && obPrices[j].VALUE > 0)
				{
					...
					strPrice += '<div class="offers_price'+(arStikePrices ? (BX.util.in_array(obPrices[j].PRICE_ID, arStikePrices) ? ' strike_block' : '') : '')+'"><a class="link-modal" data-fancybox data-src="#price-modal" href="javascript:;">?</a>';
...
$(document).ready(function () {
	...
	/*var $window = $(window),
		$menu = $('.catalog_detail .fixed-menu'),
		$bottom = $('#footer'),
		$top = $menu.offset().top,
