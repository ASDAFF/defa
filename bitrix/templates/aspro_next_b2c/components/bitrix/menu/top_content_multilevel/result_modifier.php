...
if($arResult){
	...
	if($isCatalog){
		$catalog_id=\Bitrix\Main\Config\Option::get("aspro_next_b2c", "CATALOG_IBLOCK_ID", CNextCacheB2c::$arIBlocks[SITE_ID]['aspro_next_catalog']['aspro_next_catalog'][0]);
		$arSections = CNextCacheB2c::CIBlockSection_GetList(array('SORT' => 'ASC', 'ID' => 'ASC', 'CACHE' => array('TAG' => CNextCacheB2c::GetIBlockCacheTag($catalog_id), 'GROUP' => array('ID'))), array('IBLOCK_ID' => $catalog_id, 'ACTIVE' => 'Y', 'GLOBAL_ACTIVE' => 'Y', 'ACTIVE_DATE' => 'Y', '<DEPTH_LEVEL' =>\Bitrix\Main\Config\Option::get("aspro_next_b2c", "MAX_DEPTH_MENU", 2)), false, array("ID", "NAME", "PICTURE", "LEFT_MARGIN", "RIGHT_MARGIN", "DEPTH_LEVEL", "SECTION_PAGE_URL", "IBLOCK_SECTION_ID"));
		$arTmpResult = array();
		if($arSections){
