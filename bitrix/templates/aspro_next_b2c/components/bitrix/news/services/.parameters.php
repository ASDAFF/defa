...
if(\Bitrix\Main\Loader::includeModule('aspro.next')){
	$arPageBlocks = CNextB2c::GetComponentTemplatePageBlocks(__DIR__);
	$arPageBlocksParams = CNextB2c::GetComponentTemplatePageBlocksParams($arPageBlocks);
	CNextB2c::AddComponentTemplateModulePageBlocksParams(__DIR__, $arPageBlocksParams); // add option value FROM_MODULE

	$landingIBlockID = CNextCacheB2c::$arIBlocks[$_REQUEST["src_site"]]["aspro_next_catalog"]["aspro_next_catalog_info"][0];
	$tizerIBlockID = CNextCacheB2c::$arIBlocks[$_REQUEST["src_site"]]["aspro_next_content"]["aspro_next_tizers"][0];
}
