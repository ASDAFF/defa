...
if ($arResult["VARIABLES"]["SECTION_ID"] > 0) {
    $section = CNextCache::CIBlockSection_GetList(array(
        'CACHE' => array(
            "MULTI" => "N",
...
if ($section) {
    ...
    if ($section[$arParams["SECTION_DISPLAY_PROPERTY"]]) {
        $arDisplayRes = CUserFieldEnum::GetList(array(),
    ...
    $iSectionsCount = CNextCache::CIBlockSection_GetCount(array('CACHE' => array("TAG" => CNextCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))),
        array("SECTION_ID" => $arSection["ID"], "ACTIVE" => "Y", "GLOBAL_ACTIVE" => "Y"));
    ...
    $itemsCnt = CNextCache::CIBlockElement_GetList(array("CACHE" => array("TAG" => CNextCache::GetIBlockCacheTag($arParams["IBLOCK_ID"]))),
        $arElementFilter, array());
    ...
    if ($section['UF_OFFERS_TYPE']) {
        ...
        if ($section["DEPTH_LEVEL"] > 2) {
            $sectionParent = CNextCache::CIBlockSection_GetList(array(
                'CACHE' => array(
                    "MULTI" => "N",
                    "TAG" => CNextCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])
                )
                ...
                "IBLOCK_ID" => $arParams["IBLOCK_ID"]
            ), false, array("ID", "IBLOCK_ID", "NAME", "UF_OFFERS_TYPE"));
            if ($sectionParent['UF_OFFERS_TYPE'] && !$typeTmpSKU) {
                $typeTmpSKU = $sectionParent['UF_OFFERS_TYPE'];
            }

            if (!$typeTmpSKU) {
                $sectionRoot = CNextCache::CIBlockSection_GetList(array(
                    'CACHE' => array(
                        "MULTI" => "N",
                        "TAG" => CNextCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])
                    )
...
<?
//seo
$arSeoItems = CNextCache::CIBLockElement_GetList(array(
    'CACHE' => array(
        "MULTI" => "Y",
...
//премищуства
    ...
    $res = CIBlockElement::GetList(array(), array(
        ...
        "IBLOCK_ID" => $arParams["IBLOCK_ID"],
        "ACTIVE" => "Y"
    ), false, false, array("ID", "IBLOCK_ID", "NAME", "PROPERTY_TYPE_PRODUCT"));
    ...
    while ($arOffer = $rsOffers->getNext()) {
        ...
        foreach ($arParams['SERIES_LIST_COLORS_ADD_PROPS'] as $prop) {
            if ($prop && $arOffer["PROPERTY_" . $prop . "_VALUE"] && !in_array($arOffer["PROPERTY_" . $prop . "_VALUE"],
                    $arColorItem["COLOR_" . $prop])) {
    ...
    $result = $entityClass::getList(array(
        ...
        'filter' => array("UF_XML_ID" => $arColorId)
    ));
                ...
                <div class="col-md-12 name-wrapper">
                    <div class="name">
                        <a href="<?= $arSection['SECTION_PAGE_URL']; ?>"
                           class="dark_link">Серия <?= $arSection['NAME']; ?> <span
                                    class="dark_link-opacity">|</span> <?= $section["UF_NAME_RUS"] ?></a>
                    </div>
                    <div class="name-middle">
                        ...
                        <div class="line_block share top">
                            <? $APPLICATION->IncludeFile(SITE_DIR . "include/share_buttons.php", Array(),
                        ...
                        <div class="color-wrapper">
                            <div class="kind-property">
                                ...
                                <? foreach ($section["COLORS"]["COLOR"] as $color) { ?>
                                    ...
                                    <a class="color-item" href="javascript:;"
                                       style="background: url(<?= $color['FILE_SRC'] ?>)"></a>
                                <? } ?>
                            ...
                            <a href="#moreInformSeries" class="show-more-colors" id="showMeMoreColors">Показать все
                                цвета <i class="fa fa-angle-double-down fa-fw"></i></a>
                        </div>


                        <p>Нужна серия в другом цвете? <span class="animate-load request-new-color" data-event="jqm"
                                                             data-param-form_id="SIMPLE_FORM_11"
                                                             data-name="SIMPLE_FORM_11"
                                                             data-autoload-product_name="<?= CNextB2c::formatJsName($arResult["NAME"]); ?>"
                                                             data-autoload-product_id="<?= $arResult["ID"]; ?>">Оставить заявку</span>
                        </p>
                    </div>
                </div>
            </div>
                            ...
                            <div class="col-md-12 col-lg-12">
                                ...
                                <div class="row">
                                    <div class="col-lg-12 another-color">
                                        ...
                                        <a class="btn"><span class="animate-load" data-event="jqm"
                                                             data-param-form_id="SIMPLE_FORM_11"
                                                             data-name="SIMPLE_FORM_11"
                                                             data-autoload-product_name="<?= CNextB2c::formatJsName($arResult["NAME"]); ?>"
                                                             data-autoload-product_id="<?= $arResult["ID"]; ?>">Оставить заявку</span></a>
                                    </div>
                                </div>
                            </div>
                        </div>
