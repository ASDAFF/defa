...
if($arResult['ITEMS'])
{
    ...
    $intCurRegion = \CNextRegionalityB2c::getCurrentRegion()['ID'];

    foreach ($arResult['ITEMS'] as &$arItem)
    {
        if( $arItem['PROPERTIES']['REGION']['VALUE'] != $intCurRegion )
        {
            $arItem = '';
        }
    }
	...
	{
		CNextB2c::getFieldImageData($arItem, array('PREVIEW_PICTURE'));
