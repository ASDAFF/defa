...
if (!empty($arResult['ITEMS'])){
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			if ('TYPE_1' == $arParams['TYPE_SKU'] && $arParams['DISPLAY_TYPE'] !='table')
			{
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					$arAddToBasketData = CNextB2c::GetAddToBasketArray($arOffer, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'small read_more1', $arParams);
					...
					$arOneRow = array(
						...
						'AVAILIABLE' => CNextB2c::GetQuantityArray($totalCount),
				...
				{
					$arItem['MIN_PRICE'] = CNextB2c::getMinPriceFromOffersExt(
						$arItem['OFFERS'],
		...
		else
		{
			if($arParams['USE_PRICE_COUNT'] == 'Y')
			{
				$arItem["FIX_PRICE_MATRIX"] = CNextB2c::checkPriceRangeExt($arItem);
			}
			//format prices when USE_PRICE_COUNT
			$arItem = array_merge($arItem, CNextB2c::formatPriceMatrix($arItem));
		}
