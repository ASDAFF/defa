...
else
{
	...
	{
		$sectionParent = CNextCacheB2c::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arSection["IBLOCK_SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "UF_ELEMENT_DETAIL"));
...
<?CNextB2c::checkBreadcrumbsChain($arParams, $arSection, $arElement);?>
...
if(!in_array($sViewElementTemplate, $arWidePage)):
/*similar goods*/
$arExpValues=CNextCacheB2c::CIBlockElement_GetProperty($arParams["IBLOCK_ID"], $ElementID, array("CACHE" => array("TAG" =>CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array("CODE" => "EXPANDABLES"));
if($arExpValues){
	$arAllValues["EXPANDABLES"]=$arExpValues;
}
/*accessories goods*/
$arAccessories=CNextCacheB2c::CIBlockElement_GetProperty($arParams["IBLOCK_ID"], $ElementID, array("CACHE" => array("TAG" =>CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array("CODE" => "ASSOCIATED"));
if($arAccessories){
