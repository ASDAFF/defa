...
if(!empty($arResult['SECTION']['UF_SERIES']))
{
    ...
    if(!empty($sectionsIds))
    {
        ...
        if(!empty($productsIds))
        {
            ...
            if(!empty($tree))
            {
                ...
                (
                    ...
                    false,
                    false,
                    ['ID','DETAIL_PAGE_URL','PROPERTY_MORE_PHOTO','PROPERTY_COLOR_REF','PROPERTY_'.$skyPropId]//PROPERTY_COLOR_REF
                );
        ...
        (
            ...
            false,
            false,
            ['ID','SORT','PREVIEW_PICTURE','DETAIL_PICTURE','DETAIL_PAGE_URL','PROPERTY_TYPE_PRODUCT']
        );
        $productsIds=[];
        $products=[];
        while($el=$res->GetNext())
        {
            ...
            [
                'ID'=>$el['ID'],
                'SORT'=>$el['SORT'],
                'PREVIEW_PICTURE'=>CFile::GetPath($el['PREVIEW_PICTURE']),
                'DETAIL_PICTURE'=>CFile::GetPath($el['DETAIL_PICTURE']),
                'DETAIL_PAGE_URL'=>$el['DETAIL_PAGE_URL'],
                'PROPERTY_TYPE_PRODUCT_VALUE'=>$el['PROPERTY_TYPE_PRODUCT_VALUE'],
            ];
        }
        if(!empty($productsIds))
        {
            ...
            while($el=$res->fetch())
            {
                if(in_array($el['ID'],$sectionsIds))
                {
                    $tree[$el['ID']][$el['IBLOCK_ELEMENT_ID']]=$products[$el['IBLOCK_ELEMENT_ID']];
                }
            }
            foreach($tree as &$products)
            {
                if(empty($products))
                {
                    unset($products);
                ...
                {
                    usort($products,function($a,$b){return($a['SORT']-$b['SORT']);});
                }
            }
			unset($products);
