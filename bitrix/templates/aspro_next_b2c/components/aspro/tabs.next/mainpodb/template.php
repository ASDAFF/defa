...
$this->setFrameMode(true);
	...
	if($arParams["LINE_ELEMENT_COUNT"]>=3 && $arParams["LINE_ELEMENT_COUNT"]<4)
		$col=3;
	if ($arResult["GROUP_PODBOR"]){
		foreach($arResult["GROUP_PODBOR"] as $code=> $arTab){?>
			<li data-code="<?=$code?>" class="<?=($i==1 ? "cur" : "")?>"  data-filter="<?=($arTab["FILTER"] ? urlencode(serialize($arTab["FILTER"])) : '');?>" style="display: inline-block; margin-right: 20px"><span><?=$arTab["TITLE"];?></span></li>
		<?}
