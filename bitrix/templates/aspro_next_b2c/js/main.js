...
scrollToBlock = function(block){
	if($(block).length)
	{
		var offset = $(block).offset().top - 200;
...
MegaMenuFixed = function(){
	...
	$('.bottom-menu-block .top-block-item .burger').on('click', function(){
...
$(document).ready(function(){
		...
		if(!$(this).hasClass('active')){
			...
			if(parent.hasClass('ajax')) // front tabs
			{
				$.ajax({
					...
					type: 'POST',
					data: {AJAX_POST: 'Y', AJAX_PARAMS: $(this).closest('.tab_slider_wrapp').find('.request-data').data('value'), GLOBAL_FILTER: $(this).data('filter')},
				}).success(function(html){

					//curLi.find('.tabs_slider').html(html);
					$('.hit-block .tabs_content').html(html);
					...
					/*equal height*/
					$('.hit-block .catalog_block .catalog_item_wrapp .catalog_item .item_info:visible .item-title').sliceHeight({item:'.catalog_item:visible', mobile: true});
                    $('.hit-block .catalog_block .catalog_item_wrapp .catalog_item .item_info:visible').sliceHeight({item:'.catalog_item:visible', mobile: true});
                    $('.hit-block .catalog_block .catalog_item_wrapp .catalog_item:visible').sliceHeight({classNull: '.footer_button', item:'.catalog_item:visible', mobile: true});
	...
	$('.tabs_podborki>li').on('click', function(){
		...
		if(!$(this).hasClass('active')){
			...
			if(parent.hasClass('ajax')) // front tabs
			{
				$.ajax({
					...
					//curLi.find('.tabs_slider').html(html);
					$('.podborki-block .tabs_content').html(html);
					...
					/*equal height*/
					$('.podborki-block .catalog_block .catalog_item_wrapp .catalog_item .item_info:visible .item-title').sliceHeight({item:'.catalog_item:visible', mobile: true});
                    $('.podborki-block .catalog_block .catalog_item_wrapp .catalog_item .item_info:visible').sliceHeight({item:'.catalog_item:visible', mobile: true});
                    $('.podborki-block .catalog_block .catalog_item_wrapp .catalog_item:visible').sliceHeight({classNull: '.footer_button', item:'.catalog_item:visible', mobile: true});

					initCountdown();



					$(".podborki-block .top_wrapper .catalog_block").slick({
						slidesToShow: 5,
						slidesToScroll: 1,
						dots: false,
						arrows: true,
...
$('.tabs_hit>li').on('click', function(){
		...
		if(!$(this).hasClass('active')){
			...
			var sliderIndex = $(this).index(),
				...
				curLi=$(this).closest('.top_blocks').siblings('.tabs_content').find('>li:eq(' + sliderIndex + ')');
			$(this).addClass('active').addClass('cur').siblings().removeClass('active').removeClass('cur');
console.info('ona!!');			//curLi.addClass('cur').siblings().removeClass('cur');
			curLiNav.addClass('cur').siblings().removeClass('cur');
