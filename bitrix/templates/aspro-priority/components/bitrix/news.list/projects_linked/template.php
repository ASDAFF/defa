...
<?if($arResult['ITEMS']):?>
	<div class="item-views news-items projects type_1 front linked fixed">
		...
		?>
		<div class="flexslider unstyled row dark-nav" data-plugin-options='{"itemMargin": 32, "directionNav": true, "controlNav" :false, "animationLoop": true, <?=($bAnimation ? '"slideshow": true,' : '"slideshow": false,')?> <?=($slideshowSpeed >= 0 ? '"slideshowSpeed": '.$slideshowSpeed.',' : '')?> <?=($animationSpeed >= 0 ? '"animationSpeed": '.$animationSpeed.',' : '')?> "counts": [2, 2, 2, 1]}'>
