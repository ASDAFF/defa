...
{
	$arSections = $arSectionsID = $arResult['SECTIONS_ICON'] = array();
	...
	if($arParams['SHOW_ICONS']){
		$arResult['SECTIONS_ICON'] = CCache::CIBLockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('ID'), 'MULTI' => 'N')), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'ID' => $arSectionsID), false, array('ID', 'UF_ICON'));
	}
	...
	if($arParams['SHOW_CHILD_SECTIONS'] == 'Y')
	{
		...
		{*/
			...
			{
				$arItems = CCache::CIBlockElement_GetList(array('SORT' => 'ASC', 'ID' => 'DESC', 'CACHE' => array('TAG' => CCache::GetIBlockCacheTag($arParams['IBLOCK_ID']))), array_merge($arSectionsFilter, array('SECTION_ID' => $arSection['ID'])), false, array(), array('ID', 'NAME', 'IBLOCK_ID', 'DETAIL_PAGE_URL'));
				if($arItems)
				{
