...
if($arResult['SECTIONS'])
{
	...
	// fill elements
	if($arParams['FILTER_NAME'] == 'arRegionLink' && $arTheme['SHOW_SECTIONS_REGION']['VALUE'] == 'Y'){
	...
	foreach($arSections as $key => $arSection)
	{
		...
		$arItems = CCache::CIBlockElement_GetList(array('SORT' => 'ASC', 'ID' => 'DESC', 'CACHE' => array('TAG' => CCache::GetIBlockCacheTag($arParams['IBLOCK_ID']))), array_merge($arSectionsFilter, $arItemsFilter), false, false, array('ID', 'NAME', 'IBLOCK_ID', 'DETAIL_PAGE_URL', 'PROPERTY_LINK_REGION'));
		if($arItems)
		{
			...
			if($arParams['FILTER_NAME'] == 'arRegionLink' && $arRegion && $arTheme['USE_REGIONALITY']['VALUE'] === 'Y' && $arTheme['USE_REGIONALITY']['DEPENDENT_PARAMS']['REGIONALITY_FILTER_ITEM']['VALUE'] === 'Y'){
				foreach($arSections[$key]['CHILD'] as $key2 => $arItem){
					if(is_array($arItem['PROPERTY_LINK_REGION_VALUE']) && !in_array($arRegion['ID'], $arItem['PROPERTY_LINK_REGION_VALUE'])){
						unset($arSections[$key]['CHILD'][$key2]);
					}
					else{
						if($arItem['PROPERTY_LINK_REGION_VALUE'] != $arRegion['ID']){
							unset($arSections[$key]['CHILD'][$key2]);
						}
					}
				}
		...
		elseif($arParams['FILTER_NAME'] == 'arRegionLink' && $arRegion && $arTheme['USE_REGIONALITY']['VALUE'] === 'Y' && $arTheme['USE_REGIONALITY']['DEPENDENT_PARAMS']['REGIONALITY_FILTER_ITEM']['VALUE'] === 'Y' && $arTheme['SHOW_SECTIONS_REGION']['VALUE'] == 'Y'){
