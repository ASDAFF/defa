...
$arResult['SECTIONS'] = CCache::CIBLockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('ID'), 'MULTI' => 'N')), $arSectionsFilter, false, array('ID', 'NAME', 'IBLOCK_ID', 'DEPTH_LEVEL', 'IBLOCK_SECTION_ID', 'SECTION_PAGE_URL', 'UF_ICON', 'DESCRIPTION', 'UF_BACKGROUND'));
...
if($arResult['SECTIONS'])
{
	...
	if($arTheme['SHOW_SECTIONS_REGION']['VALUE'] == 'Y'){
		$arItemsFilter['PROPERTY_LINK_REGION'] = $arRegion['ID'];
	}
	
	$arItemsFilter['INCLUDE_SUBSECTIONS'] = 'Y';
	
	if($arTheme['SHOW_SECTIONS_REGION']['VALUE'] == 'Y' && $arRegion && $arTheme['USE_REGIONALITY']['VALUE'] === 'Y' && $arTheme['USE_REGIONALITY']['DEPENDENT_PARAMS']['REGIONALITY_FILTER_ITEM']['VALUE'] === 'Y'){
		foreach($arResult['SECTIONS'] as $arSection){
			$arItemsFilter['SECTION_ID'] = $arSection['ID'];
			$arItems = CCache::CIBlockElement_GetList(array('SORT' => 'ASC', 'ID' => 'DESC', 'CACHE' => array('TAG' => CCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('IBLOCK_SECTION_ID'))), array_merge($arSectionsFilter, $arItemsFilter), array(), false, array('ID', 'IBLOCK_SECTION_ID'));
			if(!$arItems){
