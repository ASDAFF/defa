...
$arElement = CNextCacheStock::CIblockElement_GetList(array('CACHE' => array('TAG' => CNextCacheStock::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'N')), $arItemFilter, false, false, array('ID', 'PREVIEW_TEXT', 'IBLOCK_SECTION_ID', 'PREVIEW_PICTURE', 'DETAIL_PICTURE', 'PROPERTY_LINK_GOODS', 'PROPERTY_LINK_REGION'));
...
if($arParams["SHOW_NEXT_ELEMENT"] == "Y")
{
	...
	$arAllElements = CNextCacheStock::CIblockElement_GetList(array($arParams["SORT_BY1"] => $arParams["SORT_ORDER1"], $arParams["SORT_BY2"] => $arParams["SORT_ORDER2"], 'CACHE' => array('TAG' => CNextCacheStock::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "ACTIVE" => "Y", "SECTION_ID" => $arElement["IBLOCK_SECTION_ID"]/*, ">ID" => $arElement["ID"]*/ ), false, false, array('ID', 'DETAIL_PAGE_URL', 'IBLOCK_ID', 'SORT'));
	if($arAllElements)
	{
...
<?else:?>
	...
	<div class="detail <?=($templateName = $component->{'__template'}->{'__name'})?>">
		...
		?>
		<?$arSections = CNextCacheStock::CIBLockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CNextCacheStock::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('ID'), 'MULTI' => 'N', 'URL_TEMPLATE' => $arResult['FOLDER'].$arResult['URL_TEMPLATES']['section'])), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'DEPTH_LEVEL' => 1, 'ACTIVE' => 'Y'), false, array('ID', 'SECTION_PAGE_URL'));
		$arTags = array();
		if($arSections)
		{
			foreach($arSections as $key => $arSection)
			{
				$arElements = CNextCacheStock::CIblockElement_GetList(array('CACHE' => array('TAG' => CNextCacheStock::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), array_merge($arItemFilter, array("SECTION_ID" => $arSection["ID"])), false, false, array('ID', 'TAGS'));
				if(!$arElements)
		...
		{
			$arElements = CNextCacheStock::CIblockElement_GetList(array('CACHE' => array('TAG' => CNextCacheStock::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), $arItemFilter, false, false, array('ID', 'TAGS'));
		...
		<?$this->__component->__template->SetViewTarget('under_sidebar_content');?>
			...
			<?if($arParams['ALSO_ITEMS_POSITION'] == 'side'):?>
				<?$APPLICATION->IncludeComponent("bitrix:news.list", "items-blog-list", array(
					...
					"IBLOCK_ID" => CNextCacheStock::$arIBlocks[SITE_ID]["aspro_next_content"]["aspro_next_articles"][0],
