...
<?
$arItemsFilter = CNextStock::GetIBlockAllElementsFilter($arParams);

if(!($bMap = in_array('MAP', $arParams['LIST_PROPERTY_CODE']))){
	$itemsCnt = CNextCacheStock::CIBlockElement_GetList(array('CACHE' => array('TAG' => CNextCacheStock::GetIBlockCacheTag($arParams['IBLOCK_ID']))), $arItemsFilter, array());
}
else{
	// get items & coordinates
	$arItems = CNextCacheStock::CIBlockElement_GetList(array('CACHE' => array('TAG' => CNextCacheStock::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'URL_TEMPLATE' => $arResult['FOLDER'].$arResult['URL_TEMPLATES']['detail'])), $arItemsFilter, false, false, array('ID', 'NAME', 'DETAIL_PAGE_URL', 'PREVIEW_TEXT', 'PROPERTY_ADDRESS', 'PROPERTY_PHONE', 'PROPERTY_EMAIL', 'PROPERTY_SCHEDULE', 'PROPERTY_METRO', 'PROPERTY_MAP'));
