...
$productSlider = CNextStock::getSliderForItemExt($arResult, $arParams['ADD_PICT_PROP'], 'Y' == $arParams['ADD_DETAIL_TO_SLIDER']);
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		$offerSlider = CNextStock::getSliderForItemExt($arOffer, $arParams['OFFER_ADD_PICT_PROP'], true); // $arParams['ADD_DETAIL_TO_SLIDER'] == 'Y'
	...
	$postfix = '';
	global $arSite;
	if(\Bitrix\Main\Config\Option::get("aspro_next_stock", "HIDE_SITE_NAME_TITLE", "N")=="N")
	...
	if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
		{
			...
			$totalCount = CNextStock::GetTotalCount($arOffer, $arParams);
			$arOffer['IS_OFFER'] = 'Y';
			...
			if($arParams['USE_PRICE_COUNT'] == 'Y')
			{
				...
				$arOffer = array_merge($arOffer, CNextStock::formatPriceMatrix($arOffer));
				$sPriceMatrix = CNextStock::showPriceMatrix($arOffer, $arParams, $arOffer['~CATALOG_MEASURE_NAME']);
			}

			$arAddToBasketData = CNextStock::GetAddToBasketArray($arOffer, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'btn-lg w_icons', $arParams);
			...
			$arOneRow = array(
				...
				'AVAILIABLE' => CNextStock::GetQuantityArray($totalCount, array(), "Y"),
	...
	/*set min_price_id*/
	if('TYPE_1' != $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		$arResult['MIN_PRICE'] = CNextStock::getMinPriceFromOffersExt(
			$arResult['OFFERS'],
		...
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer){
			...
			if($arParams['USE_PRICE_COUNT'] == 'Y')
			{
				...
				$arResult['OFFERS'][$keyOffer] = array_merge($arOffer, CNextStock::formatPriceMatrix($arOffer));
			}
		}
	...
	if($arParams['USE_PRICE_COUNT'] == 'Y')
	{
		if($arResult['OFFERS'])
		{
			foreach($arResult['OFFERS'] as $keyOffer => $arOffer)
			{
				...
				$arOffer["FIX_PRICE_MATRIX"] = CNextStock::checkPriceRangeExt($arOffer);
				$arResult['OFFERS'][$keyOffer] = array_merge($arOffer, CNextStock::formatPriceMatrix($arOffer));
			}
			$arResult['MIN_PRICE'] = CNextStock::getMinPriceFromOffersExt(
				$arResult['OFFERS'],
		...
		{
			$arResult["FIX_PRICE_MATRIX"] = CNextStock::checkPriceRangeExt($arResult);
		}
	}

	//format prices when USE_PRICE_COUNT
	$arResult = array_merge($arResult, CNextStock::formatPriceMatrix($arResult));
}
...
if (intVal($arParams["IBLOCK_STOCK_ID"])){
	...
	$arResult["STOCK"]=CNextCacheStock::CIBLockElement_GetList(array('CACHE' => array("TAG" => CNextCacheStock::GetIBlockCacheTag($arParams["IBLOCK_STOCK_ID"]))), array("IBLOCK_ID" => $arParams["IBLOCK_STOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "PROPERTY_LINK_GOODS" => $arResult["ID"]), false, false, $arSelect);
}

/*services*/
if( !empty($arResult["PROPERTIES"]["SERVICES"]["VALUE"]) ){
	$arSelect = array("ID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "DETAIL_PAGE_URL");
	$arResult["SERVICES"]=CNextCacheStock::CIBLockElement_GetList(array('CACHE' => array("TAG" => CNextCacheStock::GetIBlockCacheTag($arResult["PROPERTIES"]["SERVICES"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arResult["PROPERTIES"]["SERVICES"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "ID" => $arResult["PROPERTIES"]["SERVICES"]["VALUE"]), false, false, $arSelect);
}

/*brand item*/
$arBrand = array();
if(strlen($arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]) && $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]){
	$arBrand = CNextCacheStock::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCacheStock::GetIBlockCacheTag($arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "ID" => $arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]), false, false, array("ID", "NAME", "DETAIL_PAGE_URL", "PREVIEW_PICTURE", "PREVIEW_TEXT"));
	if($arBrand){
...
/*stores product*/
$arStores=CNextCacheStock::CCatalogStore_GetList(array(), array("ACTIVE" => "Y"), false, false, array());
...
if($arResult["SECTION"])
{
	$arSection = CNextCacheStock::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCacheStock::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arResult["SECTION"]["ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "LEFT_MARGIN", "RIGHT_MARGIN", "NAME", "UF_TABLE_SIZES"));
	...
	if(!$tmpTableSizes)
	{
		...
		{
			$sectionRoot = CNextCacheStock::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCacheStock::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "<=LEFT_BORDER" => $arSection["LEFT_MARGIN"], ">=RIGHT_BORDER" => $arSection["RIGHT_MARGIN"], "DEPTH_LEVEL" => 1, "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "UF_TABLE_SIZES"));
