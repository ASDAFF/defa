...
set_time_limit(0);
$ft = fopen ($_SERVER['DOCUMENT_ROOT']."/yandex333222.log", "a+");
fwrite($ft, "start\n");
...
if(!function_exists("yandex_values_element")){
	function yandex_values_element($idElemnt){

		$arElement = CIBlockElement::GetByID($idElemnt);

		if($arPropertiesElemnt = $arElement->GetNext()){
			$arSection = CIBlockSection::GetByID($arPropertiesElemnt['IBLOCK_SECTION_ID']);
			if($resSection = $arSection->GetNext())
				$arPropertiesElemnt['IBLOCK_SECTION_VALUE'] = $resSection['NAME'];

			return $arPropertiesElemnt;
		}
		return null;
	}
}
...
if (empty($arRunErrors))
{
	...
	fwrite($fp, '<platform>1C-Bitrix!1!</platform>'."\n");
	...
	if ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_CATALOG || $arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_OFFERS)
	{
		$arSelect = array(
			...
			"PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "LANG_DIR", "DETAIL_PAGE_URL",
			"CATALOG_AVAILABLE"
		);
		...
		while ($obElement = $res->GetNextElement())
		{
			...
			preg_match_all('#<categoryId>(.+?)</categoryId>#is', $strTmpOff_tmp, $arr);
			$categoryID = $arr[1][0];
			$resSection = CIBlockSection::GetByID($categoryID);
			$strValue = '';
			$categoryName = '';
			...
			foreach ($arYandexFields as $key)
			{
				...
				case 'title':
					if (!$fieldsExist || !isset($fields[$key]))
					{
						...
						$strTmpOff.= "<".$key.">".yandex_text2xml($categoryName, true) . " " .yandex_text2xml($arAcc["~NAME"], true)."</".$key.">\n";
					}
					else
					{
						$strValue = '';
//						$strValue = yandex_get_value($arAcc, $key, $fields[$key], $arProperties, $arUserTypeFormat, $usedProtocol);
						/*$html = yandex_get_value($arAcc, $key, $fields[$key], $arProperties, $arUserTypeFormat, $usedProtocol);
						preg_match_all('#<'.$key.'>(.+?)</'.$key.'>#is', $html, $htmlValue);
						$htmlValue = $htmlValue[1][0];*/
						$strValue .= "<".$key.">".yandex_text2xml($categoryName, true) ." ".yandex_text2xml($arAcc["~NAME"], true)."</".$key.">\n";
						if ('' != $strValue)
							$strTmpOff .= $strValue."\n";
					}
					break;
	...
	elseif ($arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_PRODUCT || $arCatalog['CATALOG_TYPE'] == CCatalogSku::TYPE_FULL)
	{
		$arOfferSelect = array(
			...
			"PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_PAGE_URL",
		...
		}else fwrite($ft, "flag2\n");
        fwrite($ft, "flag3\n");
        fwrite($ft, "---------------------\n");
        fwrite($ft, $strOfferTemplateURL);
		$cnt = 0;
		...
		while ($obItem = $rsItems->GetNextElement())
		{
			...
			preg_match_all('#<categoryId>(.+?)</categoryId>#is', $arItem['YANDEX_CATEGORY'], $arr);
			$categoryID = $arr[1][0];
			$resSection = CIBlockSection::GetByID($categoryID);
			$strValue = '';
			$categoryName = '';
								...
								case 'title':
									if (!$fieldsExist || !isset($fields[$key]))
									{
										if (
											...
											$strOfferYandex .= "<".$key.">".yandex_text2xml($categoryName, true) ." ".yandex_text2xml($arOfferItem["~NAME"], true)."</".$key.">\n";
									}
									else
									{
										$strValue = '';
//										$strValue = yandex_get_value($arOfferItem, $key, $fields[$key], $arProperties, $arUserTypeFormat, $usedProtocol);
										/*$html = yandex_get_value($arOfferItem, $key, $fields[$key], $arProperties, $arUserTypeFormat, $usedProtocol);
										preg_match_all('#<'.$key.'>(.+?)</'.$key.'>#is', $html, $htmlValue);
										$htmlValue = $htmlValue[1][0];*/
										$strValue .= "<".$key.">".yandex_text2xml($categoryName, true) ." ".yandex_text2xml($arOfferItem["~NAME"], true)."</".$key.">\n";
										if ('' != $strValue)
...
//										$strValue = yandex_get_value($arOfferItem, $key, $fields[$key], $arProperties, $arUserTypeFormat, $usedProtocol);
										...
										preg_match_all('#<'.$key.'>(.+?)</'.$key.'>#is', $html, $htmlValue);
										$htmlValue = $htmlValue[1][0];
										$htmlValue = ( !is_null($htmlValue) && $htmlValue != '' ) ? $htmlValue : $arOfferItem["~NAME"] ;*/
										$strValue .= "<".$key.">".yandex_text2xml($categoryName, true). " " .yandex_text2xml($arOfferItem["~NAME"], true)."</".$key.">\n";
										if ('' != $strValue)
											$strOfferYandex .= $strValue."\n";
									}
				...
				foreach ($arYandexFields as $key)
				{
					...
					case 'title':
						if (!$fieldsExist || !isset($fields[$key]))
						{
							...
							$strValue = "<".$key.">".yandex_text2xml($categoryName, true) . " ".yandex_text2xml($arItem["~NAME"], true)."</".$key.">\n";
						}
						else
						{
//							$strValue = yandex_get_value($arItem, $key, $fields[$key], $arProperties, $arUserTypeFormat, $usedProtocol);
							/*$html = yandex_get_value($arItem, $key, $fields[$key], $arProperties, $arUserTypeFormat, $usedProtocol);
							preg_match_all('##is', $html, $valueHtml);
							$valueHtml = $valueHtml[1][0];*/
							$strValue .= "<".$key.">" . $categoryName . " " .yandex_text2xml($arItem["~NAME"], true)."</".$key.">\n";
							if ('' != $strValue)
								$strValue .= "\n";
						}
						break;
