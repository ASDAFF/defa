...
if(!class_exists('CPriorityRegionality'))
{
	class CPriorityRegionality{
		...
		public static function checkUseRegionality(){
			if(\Bitrix\Main\Loader::includeModule(PRIORITY_MODULE_ID))
			{
				if(isset($_SERVER['HTTP_ACCEPT_LANGUAGE']))
				{
					return CPriority::GetFrontParametrValue('USE_REGIONALITY');
				}
				else
				{
					if(CPriority::GetFrontParametrValue('REGIONALITY_TYPE') !== 'ONE_DOMAIN')
						return CPriority::GetFrontParametrValue('USE_REGIONALITY');
					else
						return 'N';
				}
		...
		public static function getRegionIBlockID(){
			...
			{
				if(isset(CCache::$arIBlocks[SITE_ID]['aspro_priority_regionality']['aspro_priority_regions'][0]) && CCache::$arIBlocks[SITE_ID]['aspro_priority_regionality']['aspro_priority_regions'][0])
				{
					$iRegionIBlockID = CCache::$arIBlocks[SITE_ID]['aspro_priority_regionality']['aspro_priority_regions'][0];
				}
				else
				{
					return;
		...
		public static function replaceSeoMarks(){
			...
			if($arRegion && $page_title)
			{
				...
				if(!CPriority::IsMainPage())
				{
					$bShowSiteName = (\Bitrix\Main\Config\Option::get(PRIORITY_MODULE_ID, "HIDE_SITE_NAME_TITLE", "N") == "N");
		...
		public static function getRegions(){
			...
			if($arRegions === NULL)
			{
				...
				if($iRegionIBlockID = self::getRegionIBlockID())
				{
					...
					$cache_id = 'aspro_priority_regions'.$iRegionIBlockID;
					...
					else
					{
						...
						$arSelect = array('ID', 'CODE', 'NAME', 'IBLOCK_ID', 'IBLOCK_SECTION_ID');
						...
						foreach(GetModuleEvents(PRIORITY_MODULE_ID, 'OnAsproRegionalityAddSelectFieldsAndProps', true) as $arEvent) // event for add to select in region getlist elements
						...
						foreach(GetModuleEvents(PRIORITY_MODULE_ID, 'OnAsproRegionalityGetElements', true) as $arEvent) // event for manipulation with region elements
