...
if(!class_exists("CAsproPriority"))
{
	class CAsproPriority{
		const MODULE_ID = \CPriority::MODULE_ID;
				...
				if($arAllMatchValues)
				{
					...
					if($arPostFields)
					{
						foreach($arPostFields as $crm => $arFields)
						{
							if($crm == 'FLOWLU')
							{
								$url = str_replace('#DOMAIN#', Option::get(self::MODULE_ID, 'DOMAIN_'.$crm, '', $SITE_ID), \Aspro\Functions\CAsproPriorityCRM::FLOWLU_PATH);
								$arFields['api_key'] = Option::get(self::MODULE_ID, 'TOKEN_FLOWLU', '', $SITE_ID);
								$arFields['ref'] = 'form:aspro-priority';
							...
							else
							{
								...
								$url = str_replace('#DOMAIN#', Option::get(self::MODULE_ID, 'DOMAIN_'.$crm, '', $SITE_ID), \Aspro\Functions\CAsproPriorityCRM::AMO_CRM_PATH);
								if(!$arFields['tags_leads'])
									$arFields['tags_leads'] = Option::get(self::MODULE_ID, 'TAGS_AMO_CRM_TITLE', '', $SITE_ID);
							}
							if(!$arFields[$name_field])
								$arFields[$name_field] = Option::get(self::MODULE_ID, 'LEAD_NAME_'.$crm.'_TITLE', \Bitrix\Main\Localization\Loc::getMessage('PRIORITY_MODULE_LEAD_NAME_'.$crm), $SITE_ID);

							$smCrmName = strtolower(str_replace('_', '', $crm));
							//log to file form request
							if(Option::get(self::MODULE_ID, 'USE_LOG_'.$crm, 'N', $SITE_ID) == 'Y')
							{
								\Aspro\Functions\CAsproPriority::set_log('crm', $smCrmName.'_create_lead_request', $arFields);
							}
							...
							{
								//auth
								$result = \Aspro\Functions\CAsproPriorityCRM::authAmoCrm($SITE_ID);
							...
							$result = \Aspro\Functions\CAsproPriorityCRM::query($url, \Aspro\Functions\CAsproPriorityCRM::$arCrmMethods[$crm]["CREATE_LEAD"], $arFieldsLead, $CURL, $DECODE);
							...
							if(isset($arCrmResult['response']))
							{
								if($crm == 'AMO_CRM' && $arCrmResult['response']['leads']) // create contact and company for amocrm
								{
									...
									if($arFields['notes_leads'])
									{
										...
										$resultNote = \Aspro\Functions\CAsproPriorityCRM::query($url, \Aspro\Functions\CAsproPriorityCRM::$arCrmMethods[$crm]["CREATE_NOTES"], $arFieldsNote, $CURL, $DECODE);
									...
									if($arCustomFields && $arCustomFields['companies'])
									{
										...
										$resultCompany = \Aspro\Functions\CAsproPriorityCRM::query($url, \Aspro\Functions\CAsproPriorityCRM::$arCrmMethods[$crm]["CREATE_COMPANY"], $arFieldsCompany, $CURL, $DECODE);
										...
										if(Option::get(self::MODULE_ID, 'USE_LOG_'.$crm, 'N', $SITE_ID) == 'Y')
										{
											\Aspro\Functions\CAsproPriority::set_log('crm', $smCrmName.'_create_company_response', $resultCompany);
										}
									...
									$resultContact = \Aspro\Functions\CAsproPriorityCRM::query($url, \Aspro\Functions\CAsproPriorityCRM::$arCrmMethods['AMO_CRM']['CREATE_CONTACT'], $arFieldsContact, $CURL, $DECODE);
									
									//log to file crm response
									if(Option::get(self::MODULE_ID, 'USE_LOG_'.$crm, 'N', $SITE_ID) == 'Y')
									{
										\Aspro\Functions\CAsproPriority::set_log('crm', $smCrmName.'_create_contact_response', json_decode($resultContact, true));
									}
							...
							if(Option::get(self::MODULE_ID, 'USE_LOG_'.$crm, 'N', $SITE_ID) == 'Y')
							{
								\Aspro\Functions\CAsproPriority::set_log('crm', $smCrmName.'_create_lead_response', $arCrmResult);
							}
						}
					}
				}
