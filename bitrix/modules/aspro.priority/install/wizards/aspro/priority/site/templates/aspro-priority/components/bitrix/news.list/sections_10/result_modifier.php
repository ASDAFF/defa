...
if($arResult['SECTIONS'])
{
	...
	if($arParams['FILTER_NAME'] == 'arRegionLink' && $arTheme['SHOW_SECTIONS_REGION']['VALUE'] == 'Y'){
		$arSectionsFilter['PROPERTY_LINK_REGION'] = $arRegion['ID'];
	}
	
	// fill elements
	foreach($arSections as $key => $arSection)
	{
		$arItems = CCache::CIBlockElement_GetList(array('SORT' => 'ASC', 'ID' => 'DESC', 'CACHE' => array('TAG' => CCache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('IBLOCK_SECTION_ID'))), array_merge($arSectionsFilter, array('SECTION_ID' => $arSection['ID'], 'INCLUDE_SUBSECTIONS' => 'Y')), array(), false, array('ID', 'IBLOCK_SECTION_ID'));
		$arSections[$key]['CNT_ELEMENTS'] = $arItems;
	}
	
	if($arParams['FILTER_NAME'] == 'arRegionLink' && $arRegion && $arTheme['USE_REGIONALITY']['VALUE'] === 'Y' && $arTheme['SHOW_SECTIONS_REGION']['VALUE'] == 'Y' && $arTheme['USE_REGIONALITY']['DEPENDENT_PARAMS']['REGIONALITY_FILTER_ITEM']['VALUE'] === 'Y'){
