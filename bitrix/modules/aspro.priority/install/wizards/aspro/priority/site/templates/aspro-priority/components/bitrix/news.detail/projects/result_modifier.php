<?
$arResult['GALLERY'] = $arResult['GALLERY_BIG'] = array();
...
if(is_array($arResult['DETAIL_PICTURE']) && !$bTopImage){
...
if(!empty($arResult['PROPERTIES']['GALLEY_BIG']['VALUE'])){
	foreach($arResult['PROPERTIES']['GALLEY_BIG']['VALUE'] as $img){
		$arResult['GALLERY_BIG'][] = array(
			'DETAIL' => ($arPhoto = CFile::GetFileArray($img)),
			'PREVIEW' => CFile::ResizeImageGet($img, array('width' => 616, 'height' => 616), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true),
...
if($arResult['GALLERY'] && $bTopImage){
	$arResult['GALLERY_BIG'] = array_merge($arResult['GALLERY_BIG'], $arResult['GALLERY']);
}
