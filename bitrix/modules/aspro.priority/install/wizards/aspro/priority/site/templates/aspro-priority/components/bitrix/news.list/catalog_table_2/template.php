		...
		<div class="items flexbox clearfix">
			<?foreach($arResult["ITEMS"] as $arItem):?>
				...
				<div class="item-wrap col-md-4 col-sm-4 col-xs-6">
					<div data-slice-block="Y" data-slice-params='{"classNull" : ".footer-button"}' class="item<?=(isset($arItem['CHARACTERISTICS']) && $arItem['CHARACTERISTICS'] ? ' wprops' : '')?><?=($bShowImage ? '' : ' wti')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>"<?=($bOrderViewBasket ? ' data-item="'.$dataItem.'"' : '')?>>
