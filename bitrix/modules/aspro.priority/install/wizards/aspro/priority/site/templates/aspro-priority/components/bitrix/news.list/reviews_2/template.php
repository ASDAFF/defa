			...
			<?foreach($arResult['SECTIONS'] as $si => $arSection):?>
				...
				<div class="row sid-<?=$arSection['ID']?> items">
					<?foreach($arSection['ITEMS'] as $i => $arItem):?>
						...
						$date = (isset($arItem['DISPLAY_ACTIVE_FROM']) && strlen($arItem['DISPLAY_ACTIVE_FROM']) ? $arItem['DISPLAY_ACTIVE_FROM'] : '');
						$bLogo = false;
						
						$bImage = strlen($arItem['~PREVIEW_PICTURE']);
						$arImage = ($bImage ? CFile::ResizeImageGet($arItem['~PREVIEW_PICTURE'], array('width' => 80, 'height' => 80), BX_RESIZE_IMAGE_EXACT, true) : array());
								...
								<div class="question border clearfix">
									...
									<div class="top-info">
										<div class="top-wrapper">
											<div class="wrap">
												<?if($post):?>
													<span class="post font_upper"><?=$post?></span>
												<?endif;?>
												<?if($post && $date):?>
													<span class="separator font_upper">&ndash;</span>
												<?endif;?>
												<?if($date):?>
