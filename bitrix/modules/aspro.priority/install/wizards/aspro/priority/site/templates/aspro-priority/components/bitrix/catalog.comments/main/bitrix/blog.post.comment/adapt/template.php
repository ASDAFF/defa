...
else
{
	...
	if(strlen($arResult["COMMENT_ERROR"])>0)
	{
		...
		<div class="blog-errors blog-note-box alert alert-danger">
			<div class="">
	...
	else
	{
			...
			if($comment["SHOW_AS_HIDDEN"] == "Y" || $comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
			{
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div class="blog-comment-info">
						...
						?>
						<div class="blog-comment-date font_upper_md"><?=$comment["DateFormated"]?></div>
					</div>
					...
					<div class="blog-comment-content">
						...
						{
							?>
							<span class="blog-comment-answer font_upper"><a href="javascript:void(0)" onclick="return showComment('<?=$comment["ID"]?>')"><?=GetMessage("B_B_MS_REPLY")?></a></span>
						...
						{
							?>
							<span class="blog-comment-parent font_upper"><a href="#<?=$comment["PARENT_ID"]?>"><?=GetMessage("B_B_MS_PARENT")?></a></span>
							<span class="blog-vert-separator"></span>
							<?
						}
						?>
						<span class="blog-comment-link font_upper"><a href="#<?=$comment["ID"]?>"><?=GetMessage("B_B_MS_LINK")?></a></span>
						<?
						if($comment["CAN_EDIT"] == "Y")
						{
							...
							<span class="blog-vert-separator"></span>
							<span class="blog-comment-edit font_upper"><a href="javascript:void(0)" onclick="return editComment('<?=$comment["ID"]?>')"><?=GetMessage("BPC_MES_EDIT")?></a></span>
							<?
						}
						...
						if(strlen($comment["urlToSpam"])>0)
						{
							?>
							<span class="blog-vert-separator"></span>
							<span class="blog-comment-delete blog-comment-spam font_upper"><a href="<?=$comment["urlToSpam"]?>" title="<?=GetMessage("BPC_MES_SPAM_TITLE")?>"><?=GetMessage("BPC_MES_SPAM")?></a></span>
							<?
						}
						if ($arParams["SHOW_RATING"] == "Y")
						{
							?>
							<span class="rating_vote_text font_upper" style="display:inline-block !important;">
							<span class="blog-vert-separator"></span>
							<?$GLOBALS["APPLICATION"]->IncludeComponent(
								"bitrix:rating.vote", $arParams["RATING_TYPE"],
								Array(
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			if($arResult["CanUserComment"])
			{
				...
				?>
				<div class="blog-add-comment"><a class="btn btn-default btn-transparent btn-lg" href="javascript:void(0)"><?=GetMessage("B_B_MS_ADD_COMMENT")?></a></div>
				<a name="0"></a>
				<?
