...
if($RIGHT >= "R"){
	...
	$GLOBALS['APPLICATION']->SetTitle(GetMessage("PRIORITY_PAGE_TITLE"));
	...
	foreach($arSites as $key => $arSite){
		...
		$rsItems = CIBlockElement::GetList(array("NAME" => "ASC"), array("ACTIVE" => "Y", "LID" => $optionsSiteID, "IBLOCK_CODE" => "aspro_priority_regions"), false, false, array("ID", "NAME", "IBLOCK_ID", "PROPERTY_MAIN_DOMAIN"));
		while($arItem = $rsItems->Fetch())
		{
			$arItems[] = $arItem;
		}
		$bGenerate = ((Option::get("aspro.priority", "REGIONALITY_TYPE", "ONE_DOMAIN", $optionsSiteID) == "SUBDOMAIN") && Option::get("aspro.priority", "USE_REGIONALITY", "N", $optionsSiteID) == "Y");
		if($bGenerate)
			$bShowGenerate = true;
		...
		$arTabs[] = array(
			...
			"OPTIONS" => array(
				"SITEMAP_URL" => array(
					"TITLE" => GetMessage("PRIORITY_MODULE_SITEMAP_URL_TITLE"),
					"DEFAULT" => ($arSite["SERVER_NAME"] ? $arSite["SERVER_NAME"] : $_SERVER["SERVER_NAME"]),
					"TYPE" => "text",
					"REQUIRED" => "Y",
				),
				"SITEMAP_NAME" => array(
					"TITLE" => GetMessage("PRIORITY_MODULE_SITEMAP_NAME_TITLE"),
					"DEFAULT" => "sitemap.xml",
					"TYPE" => "text",
					"REQUIRED" => "Y",
				),
				/*"SITEMAP_ADD_ROBOTS" => array(
					"TITLE" => GetMessage("PRIORITY_MODULE_SITEMAP_ADD_ROBOTS_TITLE"),
					"TYPE" => "checkbox",
					"DEFAULT" => "N",
				),*/
			),
					...
					foreach($arTab["OPTIONS"] as $optionCode => $arOption)
					{
						if(isset($arOption["REQUIRED"]) && $arOption["REQUIRED"] == "Y")
						{
							if(!$_POST[$optionCode."_".$optionsSiteID])
								$arErrors[] = GetMessage('PRIORITY_MODULE_FIELD_NO_VALUE', array('#FIELD#' => GetMessage('PRIORITY_MODULE_'.$optionCode.'_TITLE'), "#SITE_ID#" => $optionsSiteID));
						}						
						if($_POST[$optionCode."_".$optionsSiteID])
						{
							Option::set("aspro.priority", $optionCode, $_POST[$optionCode."_".$optionsSiteID], $optionsSiteID);							
						}							
					}
					$siteMapName =  Option::get("aspro.priority", "SITEMAP_NAME", $arTab["OPTIONS"]["SITEMAP_NAME"]["DEFAULT"], $optionsSiteID);
					$siteMapNewName =  Option::get("aspro.priority", "SITEMAP_NEW_NAME", $arTab["OPTIONS"]["SITEMAP_NEW_NAME"]["DEFAULT"], $optionsSiteID);
					$siteMapUrl =  Option::get("aspro.priority", "SITEMAP_URL", $arTab["OPTIONS"]["SITEMAP_URL"]["DEFAULT"], $optionsSiteID);
					$bExistSiteMap = (file_exists($arTab["SITE_DIR_FORMAT"].$siteMapName));
					if(!$bExistSiteMap)
						$arErrors[] = GetMessage("PRIORITY_MODULE_FILENAME_FULL", array("#FILE#" => $siteMapName));
	...
	<?if(!count($arTabs)):?>
		<div class="adm-info-message-wrap adm-info-message-red">
			<div class="adm-info-message">
				<div class="adm-info-message-title"><?=GetMessage("PRIORITY_NO_SITE_INSTALLED", array("#SESSION_ID#"=>bitrix_sessid_get()))?></div>
				<div class="adm-info-message-icon"></div>
			</div>
		</div>
	<?else:?>
		<?if($bShowGenerate):?>
			<div class="adm-info-message"><?=GetMessage("PRIORITY_MODULE_SITEMAP_INFO");?></div>
			<br>
			<br>
		<?endif;?>
		<?$tabControl->Begin();?>
		...
		foreach($arTabs as $key => $arTab)
		{
			...
			if($arTab["SITE_ID"])
			{
				$optionsSiteID = $arTab["SITE_ID"];?>
				<?if(isset($arTab["OPTIONS"]) && $arTab["OPTIONS"]):?>
					<tr class="heading"><td colspan="2"><?=GetMessage("PRIORITY_MODULE_MAIN_CONFIG")?></td></tr>
					<?foreach($arTab["OPTIONS"] as $optionCode => $arOption):?>
						<?$val = Option::get("aspro.priority", $optionCode, $arOption["DEFAULT"], $optionsSiteID);?>
						<tr>
							<td>
								<?=$arOption["TITLE"];?>							
							</td>
				...
				if(!$bExistSiteMap)
				{?>	
					<td><?=GetMessage("PRIORITY_MODULE_FILENAME", array("#FILE#" => $siteMapName))?></td>
					<td><?=GetMessage("PRIORITY_MODULE_NOT_EXISTS")?></td>
				<?}?>
				<?
				if($arTab["HAS_REGIONS"])
				{
					if($arTab["ITEMS"])
					{
						foreach($arTab["ITEMS"] as $arItem):?>
							<tr>
							<td><?=GetMessage("PRIORITY_MODULE_SITEMAP_DOMAIN", array("#DOMAIN#" => $arItem["PROPERTY_MAIN_DOMAIN_VALUE"]))?></td>
				...
				{?>
					<tr>
						<td style="width:100%;text-align:center;" colspan="2">
							<div class="adm-info-message"><?=GetMessage("PRIORITY_MODULE_SITEMAP_ERROR");?></div>
						</td>
					</tr>
				<?}?>
			<?}
		...
		if($REQUEST_METHOD == "POST" && strlen($Generate.$Apply.$RestoreDefaults) && check_bitrix_sessid())
		{
			...
			<?$tabControl->Buttons();?>
			<?if($bShowGenerate):?>
				<input <?if($RIGHT < "W") echo "disabled"?> type="submit" name="Generate" class="submit-btn adm-btn-save" value="<?=GetMessage("PRIORITY_MODULE_GENERATE_SITEMAP")?>" title="<?=GetMessage("PRIORITY_MODULE_GENERATE_SITEMAP")?>">
			<?endif;?>
