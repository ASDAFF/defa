<?
AddEventHandler('main', 'OnBuildGlobalMenu', 'OnBuildGlobalMenuHandlerPriority');
function OnBuildGlobalMenuHandlerPriority(&$arGlobalMenu, &$arModuleMenu){
	if(!defined('PRIORITY_MENU_INCLUDED')){
		define('PRIORITY_MENU_INCLUDED', true);
		...
		if($GLOBALS['APPLICATION']->GetGroupRight($moduleID) >= 'R'){
			...
			$arMenuPriority = array(
				'text' => GetMessage('PRIORITY_GLOBAL_MENU_TEXT'),
				'title' => GetMessage('PRIORITY_GLOBAL_MENU_TITLE'),
				'sort' => 10,
				'icon' => 'imi_priority',
				'page_icon' => 'pi_typography',
				'items_id' => 'global_menu_aspro_priority_items',
				"items" => array(
					array(
			...
			$arGlobalMenu['global_menu_aspro']['items']['aspro.priority'] = $arMenuPriority;
		}
	}
}
?>