...
if($RIGHT >= "R")
{
	...
	$GLOBALS['APPLICATION']->SetTitle(Loc::getMessage("PRIORITY_PAGE_TITLE"));
	...
	foreach($arSites as $key => $arSite)
	{
		...
		$arTabs[] = array(
			...
			"ITEMS" => array(
				"CONFIG" => array(
					"TITLE" => Loc::getMessage("PRIORITY_MODULE_CONFIG_AMO_CRM"),
					"ITEMS" => array(
						"DOMAIN_AMO_CRM" => array(
							"TYPE" => "text",
							"VALUE" => Option::get($moduleID, "DOMAIN_AMO_CRM", "", $optionsSiteID),
							"HINT" => Loc::getMessage("PRIORITY_MODULE_DOMAIN_HINT")
						),
						"LOGIN_AMO_CRM" => array(
							"TYPE" => "text",
						...
						"TOKEN_AMO_CRM" => array(
							...
							"HINT" => Loc::getMessage("PRIORITY_MODULE_TOKEN_HINT")
						),
					)
				),
				"LINK" => array(
					"TITLE" => Loc::getMessage("PRIORITY_MODULE_LINK_AMO_CRM"),
					"ITEMS" => array(
						...
						"AUTOMATE_SEND_AMO_CRM" => array(
							...
							"HINT" => Loc::getMessage("PRIORITY_MODULE_AUTOMATE_SEND_AMO_CRM_HINT")
						),
						"USE_LOG_AMO_CRM" => array(
							"TYPE" => "checkbox",
							"VALUE" => Option::get($moduleID, "USE_LOG_AMO_CRM", "N", $optionsSiteID),
							"HINT" => Loc::getMessage("PRIORITY_MODULE_USE_LOG_AMO_CRM_HINT")
						),
						"LEAD_NAME_AMO_CRM_TITLE" => array(
							"TYPE" => "text",
							"VALUE" => Option::get($moduleID, "LEAD_NAME_AMO_CRM_TITLE", Loc::getMessage("PRIORITY_MODULE_LEAD_NAME_AMO_CRM"), $optionsSiteID),
						),
						"TAGS_AMO_CRM_TITLE" => array(
							"TYPE" => "text",
							"VALUE" => Option::get($moduleID, "TAGS_AMO_CRM_TITLE", Loc::getMessage("PRIORITY_MODULE_TAGS_AMO_CRM"), $optionsSiteID),
						),
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$Auth.$SendCrm) && $RIGHT >= "W" && check_bitrix_sessid())
	{
		...
		if($Auth)
		{
			...
			{
				$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproPriorityCRM::AMO_CRM_PATH);

				$arPostFields = array(
					"USER_HASH" => $_POST["TOKEN"],
					"USER_LOGIN" => $_POST["LOGIN"],
				);
				echo \Aspro\Functions\CAsproPriorityCRM::query($url, \Aspro\Functions\CAsproPriorityCRM::$arCrmMethods['AMO_CRM']['AUTH'], $arPostFields, true, true);
			}
			else
			{
				$arError = array(
		...
		elseif($SendCrm)
		{
			...
			{
				$url = str_replace('#DOMAIN#', Option::get($moduleID, 'DOMAIN_AMO_CRM', '', $_POST["SITE_ID"]), \Aspro\Functions\CAsproPriorityCRM::AMO_CRM_PATH);
				$dataDeal = array(
				...
				// $result_text = \Aspro\Functions\CAsproPriorityCRM::query($url, "/private/api/v2/json/accounts/current/", $data, true, true);
				// echo $result_text;
				// \Aspro\Functions\CAsproPriority::set_log('crm', 'test_create_lead_response', json_decode($result_text, true));

				echo \Aspro\Functions\CAsproPriority::sendLeadCrmFromForm($_POST["FORM_ID"], $_POST["RESULT_ID"], 'AMO_CRM', $_POST["SITE_ID"], true, true);
			}
			else
			{
				$arError = array(
		...
		else
		{
			foreach($arTabs as $key => $arTab)
			{
				...
				{
					$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproPriorityCRM::AMO_CRM_PATH);
					$arPostFields = array(
						"USER_HASH" => $token,
						"USER_LOGIN" => $login,
					);
					$result = json_decode(\Aspro\Functions\CAsproPriorityCRM::query($url, "/private/api/auth.php?type=json", $arPostFields, true), true);
	...
	<?if(!count($arTabs)):?>
		<div class="adm-info-message-wrap adm-info-message-red">
			<div class="adm-info-message">
				<div class="adm-info-message-title"><?=Loc::getMessage("PRIORITY_NO_SITE_INSTALLED", array("#SESSION_ID#"=>bitrix_sessid_get()))?></div>
	...
	<?else:?>
		<?$tabControl->Begin();?>
		<?$bShowBtn = true;?>
		<form method="post" class="priority_options" enctype="multipart/form-data" action="<?=$APPLICATION->GetCurPage()?>?mid=<?=urlencode($mid)?>&amp;lang=<?=LANGUAGE_ID?>">
		<?=bitrix_sessid_post();?>
		<?
		foreach($arTabs as $key => $arTab)
		{
			...
			if($arTab["SITE_ID"])
			{
				...
				<?if(!$arTab["FORMS"]):?>
					<tr>
						<td colspan="2" style="width:100%;text-align:center;">
							<div class="adm-info-message"><?=Loc::getMessage("PRIORITY_MODULE_NO_FORMS");?></div>
						</td>
					</tr>
					<?continue;?>
				<?endif;?>
				<?foreach($arTab["ITEMS"] as $groupCode => $arOptions):?>
					<?if($groupCode == "LINK" && !$arOptions["ITEMS"]["ACTIVE_LINK_AMO_CRM"]["VALUE"]):?>
						<tr>
							<td colspan="2" style="width:100%;text-align:center;">
								<div class="adm-info-message"><?=Loc::getMessage("PRIORITY_MODULE_INTEGRATION_AMO_CRM");?></div>
							</td>
						</tr>
						<?continue;?>
					<?endif;?>

					<tr class="heading"><td colspan="2"><?=Loc::getMessage("PRIORITY_MODULE_".$groupCode."_AMO_CRM");?></td></tr>
						...
						<?else:?>
							<tr>
								<td class="adm-detail-content-cell-l" style="width:50%;<?=($arOption["HINT"] ? "vertical-align: top;padding-top: 7px;" : "");?>">
									<?=Loc::getMessage("PRIORITY_MODULE_".$optionCode);?>
								</td>
								<td style="width:50%;">
										...
										<td colspan="2">
											...
											$aSiteTabs = array(
												array(
													...
													"TAB" => Loc::getMessage("PRIORITY_MODULE_FIELDS_AMO_CRM"),
													"TITLE" => Loc::getMessage("PRIORITY_MODULE_ALL_FIELDS_AMO_CRM"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
												array(
													"DIV" => "edit_forms_result",
													"TAB" => Loc::getMessage("PRIORITY_MODULE_RESULTS_AMO_CRM"),
													"TITLE" => Loc::getMessage("PRIORITY_MODULE_ALL_RESULTS_AMO_CRM"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
											);
											...
											if($bIntegrationAmoCrm)
											{
												...
												//auth
												$result = \Aspro\Functions\CAsproPriorityCRM::authAmoCrm($optionsSiteID);

												if(isset($result["response"]) && ($result["response"] && isset($result["response"]["auth"]) && $result["response"]["auth"]))
												{
													$url = str_replace('#DOMAIN#', Option::get($moduleID, 'DOMAIN_AMO_CRM', '', $optionsSiteID), \Aspro\Functions\CAsproPriorityCRM::AMO_CRM_PATH);
													$result_text = \Aspro\Functions\CAsproPriorityCRM::query($url, "/private/api/v2/json/accounts/current/", array(), true, true);
																...
																if($codeGroup != "customers")
																{
																	foreach($arGroup as $arProp)
																	{
																		...
																		if(isset($arProp["enums"]))
																		{
																			...
																			{
																				\Aspro\Functions\CAsproPriorityCRM::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$keyEnum."_".$codeGroup] = (Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) ? Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) : iconv("UTF-8", LANG_CHARSET, $arProp["name"]."_".$enum));
																			}
																		}
																		else
																		{
																			\Aspro\Functions\CAsproPriorityCRM::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$codeGroup] = iconv("UTF-8", LANG_CHARSET, $arProp["name"]);
																		}
																	}
																}
															}
																				...
																				<tr>
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							...
																							<?foreach(\Aspro\Functions\CAsproPriorityCRM::$arCrmFileds["AMO_CRM"] as $groupCode => $arGroup):?>
																					...
																					<td style="width:50%;">
																						<select name="CRM_FORM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_form" style="width:300px;">
																							<option value=""><?=Loc::getMessage('FORM_FIELD_CRM_FIELDS_FORM_NAME_NO')?></option>
																							<optgroup label="...">
																								<?foreach(\Aspro\Functions\CAsproPriorityCRM::$arCrmFileds["MAIN"] as $key2 => $text):?>
														...
														<tr class="form_<?=$key;?>" <?=($key != $value ? "style='display: none;'" : '');?>>
															<td colspan="2">
																<?if(!$arValueForm || !$bIntegrationAmoCrm):?>
																	<div class="adm-info-message"><?=Loc::getMessage("PRIORITY_MODULE_NO_FORM_FIELD_MATCHING");?></div>
																...
																<?else:?>
																	<div class="adm-info-message"><?=Loc::getMessage("PRIORITY_MODULE_NO_FORM_RESULTS");?></div>
																<?endif;?>
															</td>
														</tr>
													<?endforeach;?>
					...
					<?if($value_count == 3 && $groupCode == "CONFIG"):?>
						<tr>
							<td colspan="2" style="width:100%;text-align:center;">
								<input type="submit" class="check_auth" value="<?=Loc::getMessage("PRIORITY_MODULE_CHECK_AUTH");?>"/>
		...
		if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) && check_bitrix_sessid())
		{
			...
			<input <?if($RIGHT < "W") echo "disabled"?> type="submit" name="Apply" class="submit-btn adm-btn-save" value="<?=Loc::getMessage("PRIORITY_MODULE_SAVE_OPTION")?>" title="<?=Loc::getMessage("PRIORITY_MODULE_SAVE_OPTION")?>">
			<input type="submit" name="RestoreDefaults" title="<?=Loc::getMessage("PRIORITY_MODULE_DELETE_OPTION")?>" onclick="confirm('<?=Loc::getMessage("PRIORITY_MODULE_DELETE_OPTION_TITLE")?>')" value="<?=Loc::getMessage("PRIORITY_MODULE_DELETE_OPTION_TEXT")?>">
						...
						if(_this.data('disabled') != 'disabled')
						{
							...
							$.ajax({
								...
								error: function(data){
									...
									_this.removeAttr('data-disabled');
									tr.find('.status_send').removeClass('success').addClass('error').find('span').text('error');
								}
							});
						}
					})
