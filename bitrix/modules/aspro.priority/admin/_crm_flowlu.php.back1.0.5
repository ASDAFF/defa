...
if($RIGHT >= "R")
{
	...
	foreach($arSites as $key => $arSite)
	{
		...
		$arTabs[] = array(
			...
			"ITEMS" => array(
				"CONFIG" => array(
					"TITLE" => Loc::getMessage("PRIORITY_MODULE_CONFIG_FLOWLU"),
					"ITEMS" => array(
						"DOMAIN_FLOWLU" => array(
							"TYPE" => "text",
							"VALUE" => Option::get($moduleID, "DOMAIN_FLOWLU", "", $optionsSiteID),
							"HINT" => Loc::getMessage("PRIORITY_MODULE_DOMEN_HINT")
						),
						"TOKEN_FLOWLU" => array(
							"TYPE" => "text",
				...
				"LINK" => array(
					"TITLE" => Loc::getMessage("PRIORITY_MODULE_LINK_FLOWLU"),
					"ITEMS" => array(
						...
						"AUTOMATE_SEND_FLOWLU" => array(
							...
							"HINT" => Loc::getMessage("PRIORITY_MODULE_AUTOMATE_SEND_FLOWLU_HINT")
						),
						"USE_LOG_FLOWLU" => array(
							"TYPE" => "checkbox",
							"VALUE" => Option::get($moduleID, "USE_LOG_FLOWLU", "N", $optionsSiteID),
							"HINT" => Loc::getMessage("PRIORITY_MODULE_USE_LOG_FLOWLU_HINT")
						),
						"LEAD_NAME_FLOWLU_TITLE" => array(
							"TYPE" => "text",
							"VALUE" => Option::get($moduleID, "LEAD_NAME_FLOWLU_TITLE", Loc::getMessage("PRIORITY_MODULE_LEAD_NAME_FLOWLU"), $optionsSiteID),
						),
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$Auth.$SendCrm) && $RIGHT >= "W" && check_bitrix_sessid())
	{
		...
		if($Auth)
		{
			...
			{
				$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproPriorityCRM::FLOWLU_PATH);
				$arPostFields = array(
					"api_key" => $_POST["TOKEN"]
				);
				echo \Aspro\Functions\CAsproPriorityCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields);
			}
			else
			{
				$arError = array(
		...
		elseif($SendCrm)
		{
			...
			{
				echo \Aspro\Functions\CAsproPriority::sendLeadCrmFromForm($_POST["FORM_ID"], $_POST["RESULT_ID"], 'flowlu', $_POST["SITE_ID"]);;
			}
			else
			{
				$arError = array(
		...
		else
		{
			foreach($arTabs as $key => $arTab)
			{
						...
						else
						{
							...
							if(isset($_POST[$optionCode."_".$optionsSiteID])){
								if($optionCode == 'DOMAIN_FLOWLU'){
									$_POST[$optionCode."_".$optionsSiteID] = str_replace(array('http://', 'https://', ' '), '', $_POST[$optionCode."_".$optionsSiteID]);
									if(strpos($_POST[$optionCode."_".$optionsSiteID], '.flowlu.ru') === false){
										$_POST[$optionCode."_".$optionsSiteID] = $_POST[$optionCode."_".$optionsSiteID].'.flowlu.ru';
									}
								}
				...
				if($domain && $token)
				{
					$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproPriorityCRM::FLOWLU_PATH);
					$arPostFields = array(
						"api_key" => $token
					);
					$result = json_decode(\Aspro\Functions\CAsproPriorityCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields), true);
			...
			if($arTab["SITE_ID"])
			{
				...
				<?foreach($arTab["ITEMS"] as $groupCode => $arOptions):?>
					<?if($groupCode == "LINK" && !$arOptions["ITEMS"]["ACTIVE_LINK_FLOWLU"]["VALUE"]):?>
						<tr>
							<td colspan="2" style="width:100%;text-align:center;">
								<div class="adm-info-message"><?=Loc::getMessage("PRIORITY_MODULE_INTEGRATION_FLOWLU");?></div>
							</td>
						</tr>
						<?continue;?>
					<?endif;?>

					<tr class="heading"><td colspan="2"><?=Loc::getMessage("PRIORITY_MODULE_".$groupCode."_FLOWLU");?></td></tr>
										...
										<td colspan="2">
											...
											$aSiteTabs = array(
												array(
													...
													"TAB" => Loc::getMessage("PRIORITY_MODULE_FIELDS_FLOWLU"),
													"TITLE" => Loc::getMessage("PRIORITY_MODULE_ALL_FIELDS_FLOWLU"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
												array(
													"DIV" => "edit_forms_result",
													"TAB" => Loc::getMessage("PRIORITY_MODULE_RESULTS_FLOWLU"),
													"TITLE" => Loc::getMessage("PRIORITY_MODULE_ALL_RESULTS_FLOWLU"),
													"ICON" => "settings",
													"PAGE_TYPE" => "site_settings",
												),
											);
																				...
																				<tr>
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							<?foreach(\Aspro\Functions\CAsproPriorityCRM::$arCrmFileds["FLOWLU"] as $key2 => $text):?>
