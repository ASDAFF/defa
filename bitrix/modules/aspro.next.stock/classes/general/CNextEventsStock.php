...
class CNextEventsStock{
	...
	public static function OnBeforeEventAddHandler(&$event, &$lid, &$arFields, &$message_id){
		...
		{
			if($arCurrentRegion = CNextRegionalityStock::getCurrentRegion())
			{
				...
				foreach(CNextRegionalityStock::$arSeoMarks as $mark => $field)
				{
			...
			if($arPhoneProp && $arParams['USE_PHONE_NORMALIZATION'] !='N')
			{
				...
				if($arUserResult['ORDER_PROP'][$arPhoneProp['ID']])
				{
					...
					else
					{
						...
						{
							$mask = \Bitrix\Main\Config\Option::get('aspro.next.stock', 'PHONE_MASK', '+7 (999) 999-99-99');
	...
	function OnBeforeUserUpdateHandler(&$arFields){
		...
		if($bAdminSection)
	    {
			...
			$sOneFIO = COption::GetOptionString(ASPRO_NEXT_STOCK_MODULE_ID, 'PERSONAL_ONEFIO', 'Y', $siteID);
			$sChangeLogin = COption::GetOptionString(ASPRO_NEXT_STOCK_MODULE_ID, 'LOGIN_EQUAL_EMAIL', 'Y', $siteID);
        }
		else
		{
			$arFrontParametrs = CNextStock::GetFrontParametrsValues($siteID);
	...
	static function clearBasketCacheHandler($orderID, $arFields, $arParams = array()){
		CNextCacheStock::ClearCacheByTag('sale_basket');
		...
		if(isset($arFields) && $arFields)
		{
			if(isset($arFields["ID"]) && $arFields["ID"])
			{
				...
				$arUser = $arUser = CNextCacheStock::CUser_GetList(array("SORT" => "ASC", "CACHE" => array("MULTI" => "N", "TAG" => CNextCacheStock::GetUserCacheTag($USER_ID))), array("ID" => $USER_ID), array("FIELDS" => array("ID", "PERSONAL_PHONE")));
				...
				if(self::isLandingSearchIblock($IBLOCK_ID)){
					...
					while($arSeoSearchElementQuery = $dbRes->Fetch()){
						if(strlen($query = trim($arSeoSearchElementQuery['VALUE']))){
							list($query, $hash, $arData) = \Aspro\Next\SearchQueryStock::getSentenceMeta($query);
					...
					if($arLandingSearchQuery){
						if(strlen($queryExample = \Aspro\Next\SearchQueryStock::getSentenceExampleQuery(reset($arLandingSearchQuery), LANG))){
					...
					if(CNextCacheStock::$arIBlocksInfo[$IBLOCK_ID]){
						$arSitesLids = CNextCacheStock::$arIBlocksInfo[$IBLOCK_ID]['LID'];
									...
									if($arSite){
										...
										if(!$condIblockId){
											$condIblockId = \Bitrix\Main\Config\Option::get(
												self::moduleID,
												'CATALOG_IBLOCK_ID',
												CNextCacheStock::$arIBlocks[$siteId]['aspro_next_catalog']['aspro_next_catalog'][0],
												$siteId
											);
										}
										
										if(isset(CNextCacheStock::$arIBlocksInfo[$condIblockId])){
											$pathFile = str_replace(array('#SITE_DIR#', 'index.php'), array($siteDir, ''), CNextCacheStock::$arIBlocksInfo[$condIblockId]['LIST_PAGE_URL']).'index.php';
	...
	static public function isLandingSearchIblock($IBLOCK_ID){
		return isset(CNextCacheStock::$arIBlocksInfo[$IBLOCK_ID]) && strpos(CNextCacheStock::$arIBlocksInfo[$IBLOCK_ID]['CODE'], 'aspro_next_search') !== false;
	}
	...
	static function OnEndBufferContentHandler(&$content)
	{
		if(!defined('ADMIN_SECTION') && !defined('WIZARD_SITE_ID'))
		{
				...
				if($start>0)
				{
					...
					if(($end>0) && ($end>$start))
					{
						if(defined("BX_UTF") && BX_UTF === true)
							$content = CNextStock::utf8_substr_replace($content, "", $start, $end-$start);
						else
			...
			//process recaptcha
			if(\Aspro\Functions\CAsproNextReCaptchaStock::checkRecaptchaActive())
			{
				$count = 0;
				$contentReplace = preg_replace_callback(
					...
					function ($arImage)
					{
						...
						return \Aspro\Functions\CAsproNextReCaptchaStock::callbackReplaceImage($arImage);
					},
					$html,
					-1,
					$count
				...
				$captcha_public_key = \Aspro\Functions\CAsproNextReCaptchaStock::getPublicKey();
	...
	public static function OnPageStartHandler(){
		...
		{
			if($code = \Aspro\Functions\CAsproNextReCaptchaStock::getCodeByPostList($arPostData))
			{
		...
		foreach($arPostData as $key => $arPost)
		{
			...
			if($code = \Aspro\Functions\CAsproNextReCaptchaStock::getCodeByPostList($arPost))
			{
		...
		if($needReInit)
		{
			\Aspro\Functions\CAsproNextReCaptchaStock::reInitContext($application, $request);
		}
	}
	...
	static function OnIBlockPropertyBuildListStoresHandler(){
		return array(
			...
			"GetPropertyFieldHtml"   => array("CNextEventsStock", "GetPropertyFieldHtmlStoresHandler"),
			"GetPropertyFieldHtmlMulty"   => array("CNextEventsStock", "GetPropertyFieldHtmlStoresHandlerMulty"),
		);
	}
	...
	static function OnIBlockPropertyBuildListLocationsHandler(){
		return array(
			...
			"GetPropertyFieldHtml"   => array("CNextEventsStock", "GetPropertyFieldHtmlLocationsHandler"),
		);
	}
	...
	static function OnIBlockPropertyBuildListPricesHandler(){
		return array(
			...
			"GetPropertyFieldHtml"   => array("CNextEventsStock", "GetPropertyFieldHtmlPricesHandler"),
			"GetPropertyFieldHtmlMulty"   => array("CNextEventsStock", "GetPropertyFieldHtmlPricesHandlerMulty"),
		);
	}
	...
	static function OnIBlockPropertyBuildCustomFilterHandler(){
		return array(
			...
			'GetPropertyFieldHtml'   => array('CNextEventsStock', 'GetPropertyFieldHtmlCustomFilterHandler'),
			'GetSettingsHTML'   => array('CNextEventsStock', 'GetSettingsHTMLCustomFilterHandler'),
			'PrepareSettings'   => array('CNextEventsStock', 'PrepareSettingsCustomFilterHandler'),
		);
	}
	...
	static function OnRegionUpdateHandler($arFields){
		...
		if(isset(CNextCacheStock::$arIBlocks[$arIBlock['LID']]['aspro_next_regionality']['aspro_next_regions'][0]) && CNextCacheStock::$arIBlocks[$arIBlock['LID']]['aspro_next_regionality']['aspro_next_regions'][0])
			$iRegionIBlockID = CNextCacheStock::$arIBlocks[$arIBlock['LID']]['aspro_next_regionality']['aspro_next_regions'][0];
		else
			return;
	...
	static function onBeforeResultAddHandler($WEB_FORM_ID, &$arFields, &$arrVALUES){
		...
		{
			global $APPLICATION;
			$arTheme = CNextStock::GetFrontParametrsValues(SITE_ID);
