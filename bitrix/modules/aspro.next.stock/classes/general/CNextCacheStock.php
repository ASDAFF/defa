...
if(!class_exists("CNextCacheStock")){
	class CNextCacheStock {
				...
				if($arGroupBy === array()){
					...
					if($bGetDetailPageUrlsArray){
						...
						foreach($arByIBlock as $IBLOCK_ID => $arIB){
							...
							if($arSectionIDs){
								$arSections = CNextCacheStock::CIBlockSection_GetList(array("CACHE" => array("TAG" => CNextCacheStock::GetIBlockCacheTag($IBLOCK_ID), "MULTI" => "N", "GROUP" => array("ID"))), array("ID" => $arSectionIDs), false, array("ID", "CODE", "EXTERNAL_ID", "IBLOCK_ID"));
							}
							foreach($arIB as $SECTION_ID => $arIDs){
								if($arIDs){
									$rsElements = CIBlockElement::GetList(array(), array("ID" => $arIDs), false, false, array("ID", "DETAIL_PAGE_URL", "CANONICAL_PAGE_URL"));
									$rsElements->SetUrlTemplates(CNextCacheStock::$arIBlocksInfo[$IBLOCK_ID]["DETAIL_PAGE_URL"]);
		...
		function CSaleBasket_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CNextCacheStock/sale/CSaleBasket_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		function CCatalogStore_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CNextCacheStock/catalog/CCatalogStore_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		function CForumMessage_GetListEx($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $iNum = 0, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CNextCacheStock/forum/CForumMessage_GetListEx/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		private function _InitCacheParams($moduleName, $functionName, $arCache){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CNextCacheStock/".$moduleName."/".$functionName."/".$cacheTag."/";
			}
		...
		function GetIBlockCacheTag($IBLOCK_ID){
			if(!$IBLOCK_ID){
				...
				return @CNextCacheStock::$arIBlocksInfo[$IBLOCK_ID]["CODE"].$IBLOCK_ID;
			}
		}
		...
		function ClearTagIBlockElement($arFields){
			...
			if($arFields["IBLOCK_ID"]){
				...
				{
					$arBlocks = array();
					if(CNextCacheStock::$arIBlocks)
					{
						foreach(CNextCacheStock::$arIBlocks as $arSiteIbloks)
						{
				...
				$CACHE_MANAGER->ClearByTag(CNextCacheStock::GetIBlockCacheTag($arFields["IBLOCK_ID"]));
		...
		function ClearTagIBlockSectionBeforeDelete($ID){
			global $CACHE_MANAGER;
			if($ID > 0){
				if($IBLOCK_ID = CNextCacheStock::CIBlockSection_GetList(array("CACHE" => array("MULTI" => "N")), array("ID" => $ID), false, array("IBLOCK_ID"), true)){
					$CACHE_MANAGER->ClearByTag(CNextCacheStock::GetIBlockCacheTag($IBLOCK_ID));
				}
			}
		}
	}

	// initialize CNextCacheStock::$arIBlocks array
	if(CNextCacheStock::$arIBlocks === NULL){
		$arIBlocksTmp = CNextCacheStock::CIBlock_GetList(array("CACHE" => array("TAG" => "iblocks")), array("ACTIVE" => "Y", "CHECK_PERMISSIONS" => "N"));
		CNextCacheStock::$arIBlocks = CNextCacheStock::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("LID", "IBLOCK_TYPE_ID", "CODE"), "MULTI" => "Y", "RESULT" => array("ID")));
		CNextCacheStock::$arIBlocksInfo = CNextCacheStock::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("ID")));
	}
}?>