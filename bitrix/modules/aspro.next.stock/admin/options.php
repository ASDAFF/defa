...
if($RIGHT >= "R"){
	...
	foreach($arSites as $key => $arSite){
		...
		$arTabs[] = array(
			...
			"SITE_ID" => $arSite["ID"],
			"SITE_DIR" => $arSite["DIR"],
			"TEMPLATE" => CNextStock::GetSiteTemplate($arSite["ID"]),
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) > 0 && $RIGHT >= "W" && check_bitrix_sessid()){
		...
		if(strlen($RestoreDefaults) > 0){
			Option::delete(CNextStock::moduleID);
			Option::delete(CNextStock::moduleID, array("name" => "NeedGenerateCustomTheme"));
			Option::delete(CNextStock::moduleID, array("name" => "NeedGenerateCustomThemeBG"));
			$APPLICATION->DelGroupRight(CNextStock::moduleID);
		}
		else{
			Option::delete(CNextStock::moduleID, array("name" => "sid"));
	...
	<?else:?>
		...
		if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) && check_bitrix_sessid())
		{
			...
			<?endif;?>
			<?if(CNextStock::IsCompositeEnabled()):?>
