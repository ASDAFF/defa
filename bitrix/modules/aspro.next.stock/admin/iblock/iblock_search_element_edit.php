...
if(isset($_POST['action']) && $_POST['action'] === 'getQueryExplanation'){
	...
	if(isset($_POST['query']) && strlen($_POST['query'])){
		...
		list($query, $hash, $arData) = SearchQueryStock::getSentenceMeta($query);
		if($hash !== SearchQueryStock::META_HASH_NOT_VALID){
			$example = SearchQueryStock::getSentenceExampleQuery($query);
			...
			if($hash & SearchQueryStock::META_HASH_HAS_FIXED_COUNT){
				$cntFixedCount = $hash >> 16;
				$arExplanations[] = array(
					'LOGIC' => '&&',
					'TEXT' => Loc::getMessage('QUERY_EXPLANATION_CONTAINS').' <a title="'.Loc::getMessage('QUERY_EXPLANATION_CONTAINS_EQUAL').'">'.Loc::getMessage('QUERY_EXPLANATION_EQUAL').' '.SearchQueryStock::vail($cntFixedCount, array(Loc::getMessage('QUERY_EXPLANATION_WORD1'), Loc::getMessage('QUERY_EXPLANATION_WORD2'), Loc::getMessage('QUERY_EXPLANATION_WORD0'))).'</a>',
				);
			}
			else{
				$cntAll = ($hash & (255 << 8)) >> 8;
				if($cntAll > 0){
					$arExplanations[] = array(
						'LOGIC' => '&&',
						'TEXT' => Loc::getMessage('QUERY_EXPLANATION_CONTAINS').' <a title="'.Loc::getMessage('QUERY_EXPLANATION_CONTAINS_MINIMAL').'">'.Loc::getMessage('QUERY_EXPLANATION_MINIMAL').' '.SearchQueryStock::vail($cntAll, array(Loc::getMessage('QUERY_EXPLANATION_WORD1'), Loc::getMessage('QUERY_EXPLANATION_WORD2'), Loc::getMessage('QUERY_EXPLANATION_WORD0'))).'</a>',
					);
				}
			}

			if($hash & SearchQueryStock::META_HASH_HAS_FIXED_FORMS && strlen($fixedForms)){
			...
			if($hash & SearchQueryStock::META_HASH_HAS_STOP_WORDS && strlen($stopWords)){
			...
			if($hash & SearchQueryStock::META_HASH_HAS_FIXED_ORDER && strlen($fixedOrder)){
			...
			if($hash & SearchQueryStock::META_HASH_HAS_COMPLEX && $arComplex){
			...
			if($hash & SearchQueryStock::META_HASH_HAS_MINUS_WORDS && ($minusWords['WORDS'] || $minusWords['STEM'])){
