...
if($RIGHT >= "R")
{
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$Auth.$SendCrm) && $RIGHT >= "W" && check_bitrix_sessid())
	{
		...
		if($Auth)
		{
			...
			{
				$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproNextCRMStock::AMO_CRM_PATH);

				$arPostFields = array(
					"USER_HASH" => $_POST["TOKEN"],
					"USER_LOGIN" => $_POST["LOGIN"],
				);
				echo \Aspro\Functions\CAsproNextCRMStock::query($url, \Aspro\Functions\CAsproNextCRMStock::$arCrmMethods['AMO_CRM']['AUTH'], $arPostFields, true, true);
			}
			else
			{
				$arError = array(
		...
		elseif($SendCrm)
		{
			...
			{
				$url = str_replace('#DOMAIN#', Option::get($moduleID, 'DOMAIN_AMO_CRM', '', $_POST["SITE_ID"]), \Aspro\Functions\CAsproNextCRMStock::AMO_CRM_PATH);
				$dataDeal = array(
				...
				// $result_text = \Aspro\Functions\CAsproNextCRMStock::query($url, "/private/api/v2/json/accounts/current/", $data, true, true);
				// echo $result_text;
				// \Aspro\Functions\CAsproNext::set_log('crm', 'test_create_lead_response', json_decode($result_text, true));

				echo \Aspro\Functions\CAsproNextStock::sendLeadCrmFromForm($_POST["FORM_ID"], $_POST["RESULT_ID"], 'AMO_CRM', $_POST["SITE_ID"], true, true);
			}
			else
			{
				$arError = array(
		...
		else
		{
			foreach($arTabs as $key => $arTab)
			{
				...
				{
					$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproNextCRMStock::AMO_CRM_PATH);
					$arPostFields = array(
						"USER_HASH" => $token,
						"USER_LOGIN" => $login,
					);
					$result = json_decode(\Aspro\Functions\CAsproNextCRMStock::query($url, "/private/api/auth.php?type=json", $arPostFields, true), true);
									...
									<tr>
										<td colspan="2">
											...
											if($bIntegrationAmoCrm)
											{
												...
												//auth
												$result = \Aspro\Functions\CAsproNextCRMStock::authAmoCrm($optionsSiteID);

												if(isset($result["response"]) && ($result["response"] && isset($result["response"]["auth"]) && $result["response"]["auth"]))
												{
													$url = str_replace('#DOMAIN#', Option::get($moduleID, 'DOMAIN_AMO_CRM', '', $optionsSiteID), \Aspro\Functions\CAsproNextCRMStock::AMO_CRM_PATH);
													$result_text = \Aspro\Functions\CAsproNextCRMStock::query($url, "/private/api/v2/json/accounts/current/", array(), true, true);
																...
																if($codeGroup != "customers")
																{
																	foreach($arGroup as $arProp)
																	{
																		...
																		if(isset($arProp["enums"]))
																		{
																			...
																			{
																				\Aspro\Functions\CAsproNextCRMStock::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$keyEnum."_".$codeGroup] = (Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) ? Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) : iconv("UTF-8", LANG_CHARSET, $arProp["name"]."_".$enum));
																			}
																		}
																		else
																		{
																			\Aspro\Functions\CAsproNextCRMStock::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$codeGroup] = iconv("UTF-8", LANG_CHARSET, $arProp["name"]);
																		}
																	}
																}
															}
																				...
																				<tr>
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							...
																							<?foreach(\Aspro\Functions\CAsproNextCRMStock::$arCrmFileds["AMO_CRM"] as $groupCode => $arGroup):?>
																					...
																					<td style="width:50%;">
																						<select name="CRM_FORM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_form" style="width:300px;">
																							<option value=""><?=Loc::getMessage('FORM_FIELD_CRM_FIELDS_FORM_NAME_NO')?></option>
																							<optgroup label="...">
																								<?foreach(\Aspro\Functions\CAsproNextCRMStock::$arCrmFileds["MAIN"] as $key2 => $text):?>
