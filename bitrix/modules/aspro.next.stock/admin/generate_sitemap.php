...
if($RIGHT >= "R"){
	...
	foreach($arSites as $key => $arSite){
		...
		$bGenerate = ((Option::get("aspro.next.stock", "REGIONALITY_TYPE", "ONE_DOMAIN", $optionsSiteID) == "SUBDOMAIN") && Option::get("aspro.next.stock", "USE_REGIONALITY", "N", $optionsSiteID) == "Y");
		if($bGenerate)
			$bShowGenerate = true;
			...
			foreach($arTabs as $key => $arTab)
			{
				...
				if(isset($arTab["OPTIONS"]) && $arTab["OPTIONS"])
				{
					foreach($arTab["OPTIONS"] as $optionCode => $arOption)
					{
						...
						if($_POST[$optionCode."_".$optionsSiteID])
						{
							Option::set("aspro.next.stock", $optionCode, $_POST[$optionCode."_".$optionsSiteID], $optionsSiteID);
						}
					}
					$siteMapName =  Option::get("aspro.next.stock", "SITEMAP_NAME", $arTab["OPTIONS"]["SITEMAP_NAME"]["DEFAULT"], $optionsSiteID);
					$siteMapNewName =  Option::get("aspro.next.stock", "SITEMAP_NEW_NAME", $arTab["OPTIONS"]["SITEMAP_NEW_NAME"]["DEFAULT"], $optionsSiteID);
					$siteMapUrl =  Option::get("aspro.next.stock", "SITEMAP_URL", $arTab["OPTIONS"]["SITEMAP_URL"]["DEFAULT"], $optionsSiteID);
													...
													foreach($arLandingsMatches[0] as $i => $match){
														...
														if($arLandings[$LID]){
															...
															if((!$arEnumID_IS_INDEX || ($arEnumID_IS_INDEX && $arLandings[$LID]['PROPERTY_IS_INDEX_ENUM_ID'] == $arEnumID_IS_INDEX['ID'])) && (!$arLandings[$LID]['PROPERTY_LINK_REGION_VALUE'] || in_array($arItem['ID'], $arLandings[$LID]['PROPERTY_LINK_REGION_VALUE']))){
																...
																$url = \Aspro\Next\SearchQueryStock::getLandingUrl(
																	$catalogDir,
		...
		foreach($arTabs as $key => $arTab)
		{
			...
			if($arTab["SITE_ID"])
			{
				...
				<?if(isset($arTab["OPTIONS"]) && $arTab["OPTIONS"]):?>
					...
					<?foreach($arTab["OPTIONS"] as $optionCode => $arOption):?>
						<?$val = Option::get("aspro.next.stock", $optionCode, $arOption["DEFAULT"], $optionsSiteID);?>
						<tr>
							<td>
								<?=$arOption["TITLE"];?>
							</td>
