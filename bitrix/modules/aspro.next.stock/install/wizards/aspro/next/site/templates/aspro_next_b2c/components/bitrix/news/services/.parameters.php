...
if(\Bitrix\Main\Loader::includeModule('aspro.next.stock')){
	$arPageBlocks = CNextStock::GetComponentTemplatePageBlocks(__DIR__);
	$arPageBlocksParams = CNextStock::GetComponentTemplatePageBlocksParams($arPageBlocks);
	CNextStock::AddComponentTemplateModulePageBlocksParams(__DIR__, $arPageBlocksParams); // add option value FROM_MODULE

	$landingIBlockID = CNextCacheStock::$arIBlocks[$_REQUEST["src_site"]]["aspro_next_catalog"]["aspro_next_catalog_info"][0];
	$tizerIBlockID = CNextCacheStock::$arIBlocks[$_REQUEST["src_site"]]["aspro_next_content"]["aspro_next_tizers"][0];
}
