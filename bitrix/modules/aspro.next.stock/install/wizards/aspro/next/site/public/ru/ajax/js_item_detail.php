...
<?if($arPost["PARAMS"]):?>
	...
	{
		$arPricesIDs = \Aspro\Functions\CAsproNextStock::getPricesID($arPost["PARAMS"]["PRICE_CODE"], true);
		if($arPricesIDs)
		{
	...
	$cachePath = "/CNextCacheStock/iblock/getSKUjs/".$cacheTag."/";
	$cacheTime = $arPost["PARAMS"]["CACHE_TIME"];
	// $cacheTime = 0;

	if(isset($arPost["clear_cache"]) && $arPost["clear_cache"] == "y")
		\CNextCacheStock::ClearCacheByTag($cacheTag);
	...
	else
	{
			...
			if($arOffer['PRICES'])
			{
				...
				if($arPost["PARAMS"]["USE_PRICE_COUNT"] == "Y")
				{
					if(function_exists('CatalogGetPriceTableEx'))
					{
						...
						{
							$arOffer = array_merge($arResult["ITEMS"][$key], CNextStock::formatPriceMatrix($arResult["ITEMS"][$key]));
		...
		foreach($arResult["ITEMS"] as $key => $arItem)
		{
			$arItems["ITEMS"][$key] = array(
				...
				"TOTAL_COUNT" => CNextStock::GetTotalCount($arItem, $arPost["PARAMS"])
			);
		}
	...
	if($arItems)
	{
		foreach($arItems["ITEMS"] as $key => $arItem)
		{
			...
			$arAddToBasketData = CNextStock::GetAddToBasketArray($arItem, $arItem["TOTAL_COUNT"], $arPost["PARAMS"]["DEFAULT_COUNT"], $arPost["PARAMS"]["BASKET_URL"], false, array(), 'small read_more1', $arPost["PARAMS"]);
			...
			$arItems["ITEMS"][$key]["AVAILIABLE"] = CNextStock::GetQuantityArray($arItem["TOTAL_COUNT"]);
			...
			{
				$arItems["ITEMS"][$key]["PRICE_MATRIX_HTML"] = CNextStock::showPriceMatrix($arItem, $arPost["PARAMS"], $arItem['MEASURE']);
