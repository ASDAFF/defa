...
$arItems = CNextCacheStock::CIblockElement_GetList(array("CACHE" => array("TAG" => CNextCacheStock::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), $arItemFilter, false, false, $arItemSelect);

$arAllSections = array();
if($arItems)
	$arAllSections = CNextStock::GetSections($arItems, $arParams);
?>
				...
				<div class="col-md-<?=($bHasSections ? 6 : 12);?>">
					<div class="row">
						<div class="col-md-6 print-6">
							<?CNextStock::showContactPhones('', true, '', 'Phone_black2.svg', 'grey');?>
						</div>
						<div class="col-md-6 print-6">
							<?CNextStock::showContactEmail('E-mail', true, '', 'Email.svg', 'grey');?>
						</div>
					</div>
...
<div class="ajax_items">
	...
	<?if($arItems):?>
		...
		<?
		$bUseMap = CNextStock::GetFrontParametrValue('CONTACTS_USE_MAP', SITE_ID) != 'N';
		$mapLAT = $mapLON = $iCountShops =0;
		$arPlacemarks = array();
		if($bPostSection)
		{
			$arItems = CNextCacheStock::CIblockElement_GetList(array("CACHE" => array("TAG" => CNextCacheStock::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array_merge($arItemFilter, array('SECTION_ID' => $_POST['ID'])), false, false, $arItemSelect);
