...
$oSearchQuery = new \Aspro\Next\SearchQueryStock($searchQuery);
...
if($arLanding){
	...
	if($arLanding['PROPERTY_CUSTOM_FILTER_VALUE'] && $arLanding['PROPERTY_CUSTOM_FILTER_TYPE_VALUE']){
		...
		if(\Bitrix\Main\Loader::includeModule('catalog') && class_exists('CNextConditionStock')){
			$arCustomFilter = array();
			$cond = new CNextConditionStock();
	...
	if($bReplaceElementsByCustomFilter = $arCustomFilter && $arLanding['PROPERTY_CUSTOM_FILTER_TYPE_VALUE'] && strpos($arLanding['PROPERTY_CUSTOM_FILTER_TYPE_VALUE'], GetMessage('REPLACE_SUBSTR')) !== false){
		...
		$arElements = CNextCacheStock::CIBLockElement_GetList(
			array(
				'CACHE' => array(
					'MULTI' => 'Y',
		...
		<?if($arLanding["PROPERTY_TIZERS_VALUE"]):?>
			...
			<?$APPLICATION->IncludeComponent(
				...
				array(
					...
					"IBLOCK_ID" => CNextCacheStock::$arIBlocks[SITE_ID]["aspro_next_content"]["aspro_next_tizers"][0],
					"NEWS_COUNT" => "4",
					"SORT_BY1" => "SORT",
					"SORT_ORDER1" => "ASC",
					"SORT_BY2" => "ID",
...
if (is_array($arElements) && !empty($arElements))
{
	...
	$arItems = CNextCacheStock::CIBLockElement_GetList(
		array(
			'CACHE' => array("MULTI" =>"Y", "TAG" => CNextCacheStock::GetIBlockCacheTag($catalogIBlockID))
		),
		$arItemsFilter,
		false,
		false,
	...
	if($arItems)
	{
		...
		if($arAllSections)
		{
			...
			$arSections = CNextCacheStock::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "GROUP" => "ID", "TAG" => CNextCacheStock::GetIBlockCacheTag($catalogIBlockID))), array("ID" => $arSectionsID, "IBLOCK_ID" => $catalogIBlockID), false, array("ID", "IBLOCK_ID", "NAME"));
		}?>
	...
	<?if($arLanding['PROPERTY_SIMILAR_VALUE']):?>
		...
		<?$APPLICATION->IncludeComponent(
			...
			array(
				...
				"IBLOCK_ID" => CNextCacheStock::$arIBlocks[SITE_ID]["aspro_next_catalog"]["aspro_next_search"][0],
				"NEWS_COUNT" => "999",
...
<div class="left_block filter_visible">
	...
	<?CNextStock::get_banners_position('SIDE', 'Y');?>
