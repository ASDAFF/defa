<?
	...
	{
		usort($arResult["GRID"]["ROWS"], 'CNextStock::cmpByID');
		$arImages = array();
		foreach($arResult["GRID"]["ROWS"] as $key=>$arItem)
		{
			...
			if(strpos($arItem["PRODUCT_XML_ID"], "#")!==false){
				...
				$arItem1 =CNextCacheStock::CIBLockElement_GetList(array('CACHE' => array("MULTI"=>"N", "TAG" => CNextCacheStock::GetIBlockCacheTag($arItem["IBLOCK_ID"]))), array("IBLOCK_ID" => $arItem["IBLOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "XML_ID" => $arXmlID[0]), false, false, array("ID", "IBLOCK_ID"));
		...
		usort($arResult["GRID"]["HEADERS"], 'CNextStock::cmpBySort');
