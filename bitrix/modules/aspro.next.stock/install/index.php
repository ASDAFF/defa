...
class aspro_next_stock extends CModule {
	const solutionName	= 'next.stock';
	const partnerName = 'aspro';
	const moduleClass = 'CNextStock';
	const moduleClassEvents = 'CNextEventsStock';
	const moduleClassCache = 'CNextCacheStock';

	var $MODULE_ID = "aspro.next.stock";
	var $MODULE_VERSION;
	...
	function InstallEvents(){
		...
		RegisterModuleDependences("sender", "onPresetTemplateList", $this->MODULE_ID, "\Aspro\Solution\CAsproMarketingB2c", "senderTemplateList");
	...
	function UnInstallEvents(){
		...
		UnRegisterModuleDependences("sender", "onPresetTemplateList", $this->MODULE_ID, "\Aspro\Solution\CAsproMarketingB2c", "senderTemplateList");
	...
	function removeDirectory($dir){
		if($objs = glob($dir."/*")){
			foreach($objs as $obj){
				if(is_dir($obj)){
					CNextStock::removeDirectory($obj);
				}
				else{
					if(!unlink($obj)){
						if(chmod($obj, 0777)){
	...
	function DoInstall(){
		...
		$APPLICATION->IncludeAdminFile(GetMessage("ASPRO_NEXT_SCOM_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/aspro.next.stock/install/step.php");
	}
	...
	function DoUninstall(){
		...
		$APPLICATION->IncludeAdminFile(GetMessage("ASPRO_NEXT_SCOM_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/aspro.next.stock/install/unstep.php");
	}
}
?>