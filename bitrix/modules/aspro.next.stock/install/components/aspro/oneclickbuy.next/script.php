...
else
{
	...
	$bUseCaptcha = (\Bitrix\Main\Config\Option::get('aspro.next.stock', 'ONE_CLICK_BUY_CAPTCHA', 'N') == 'Y');
	...
	if($bAllBasketBuy){
		...
		if(is_array($arOrderDat) && array_key_exists("ORDER_PRICE", $arOrderDat)){
			...
			$arError = CNextStock::checkAllowDelivery($arOrderDat["ORDER_PRICE"], $arOrderDat["CURRENCY"]);
