...
<?if(\Bitrix\Main\Loader::includeModule('aspro.next.stock'))
{
	...
	if(!$arResult['POPUP']  || ($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.next.stock', 'REGIONALITY_SEARCH_ROW', 'N') != 'Y'))
	{
		$arResult['REGIONS'] = CNextRegionalityStock::getRegions();
		$arResult['CURRENT_REGION'] = CNextRegionalityStock::getCurrentRegion();
		$arResult['REAL_REGION'] = CNextRegionalityStock::getRealRegionByIP();
	...
	if($arResult['REGIONS'])
	{
		...
		if($arResult['POPUP'])
		{
			...
			if($arSectionsID)
			{
				$arSections = CNextCacheStock::CIBlockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('GROUP' => 'ID', 'TAG' => CNextCacheStock::GetIBlockCacheTag(CNextCacheStock::$arIBlocks[SITE_ID]['aspro_next_regionality']['aspro_next_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', 'ID' => $arSectionsID, 'IBLOCK_ID' => CNextCacheStock::$arIBlocks[SITE_ID]['aspro_next_regionality']['aspro_next_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'DEPTH_LEVEL'));
				if($arSections)
				{
					$arSections2 = array();
					foreach($arSections as $key => $arSection)
					{
						...
						if($arSection['DEPTH_LEVEL'] > 1)
						{
							...
							$arTmpSection = CNextCacheStock::CIBlockSection_GetList(array('CACHE' => array('MULTI' => 'N', 'TAG' => CNextCacheStock::GetIBlockCacheTag(CNextCacheStock::$arIBlocks[SITE_ID]['aspro_next_regionality']['aspro_next_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', '<=LEFT_BORDER' => $arSection['LEFT_MARGIN'], '>=RIGHT_BORDER' => $arSection['RIGHT_MARGIN'], 'DEPTH_LEVEL' => 1, 'IBLOCK_ID' => CNextCacheStock::$arIBlocks[SITE_ID]['aspro_next_regionality']['aspro_next_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'SORT'));
	...
	elseif($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.next.stock', 'REGIONALITY_SEARCH_ROW', 'N') == 'Y')
	{
		$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/jquery-ui.min.js');
		$this->IncludeComponentTemplate();
	}
