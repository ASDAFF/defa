...
if(!class_exists("CAsproNextStock"))
{
	class CAsproNextStock{
				...
				if($arAllMatchValues)
				{
					...
					if($arPostFields)
					{
						foreach($arPostFields as $crm => $arFields)
						{
							if($crm == 'FLOWLU')
							{
								$url = str_replace('#DOMAIN#', Option::get(self::MODULE_ID, 'DOMAIN_'.$crm, '', $SITE_ID), \Aspro\Functions\CAsproNextCRMStock::FLOWLU_PATH);
							...
							else
							{
								...
								$url = str_replace('#DOMAIN#', Option::get(self::MODULE_ID, 'DOMAIN_'.$crm, '', $SITE_ID), \Aspro\Functions\CAsproNextCRMStock::AMO_CRM_PATH);
							...
							{
								//auth
								$result = \Aspro\Functions\CAsproNextCRMStock::authAmoCrm($SITE_ID);
							...
							$result = \Aspro\Functions\CAsproNextCRMStock::query($url, \Aspro\Functions\CAsproNextCRMStock::$arCrmMethods[$crm]["CREATE_LEAD"], $arFieldsLead, $CURL, $DECODE);
							...
							if(isset($arCrmResult['response']))
							{
								if($crm == 'AMO_CRM' && $arCrmResult['response']['leads']) // create contact and company for amocrm
								{
									...
									if($arFields['notes_leads'])
									{
										...
										$resultNote = \Aspro\Functions\CAsproNextCRMStock::query($url, \Aspro\Functions\CAsproNextCRMStock::$arCrmMethods[$crm]["CREATE_NOTES"], $arFieldsNote, $CURL, $DECODE);
									...
									if($arCustomFields && $arCustomFields['companies'])
									{
										...
										$resultCompany = \Aspro\Functions\CAsproNextCRMStock::query($url, \Aspro\Functions\CAsproNextCRMStock::$arCrmMethods[$crm]["CREATE_COMPANY"], $arFieldsCompany, $CURL, $DECODE);
									...
									$resultContact = \Aspro\Functions\CAsproNextCRMStock::query($url, \Aspro\Functions\CAsproNextCRMStock::$arCrmMethods['AMO_CRM']['CREATE_CONTACT'], $arFieldsContact, $CURL, $DECODE);
