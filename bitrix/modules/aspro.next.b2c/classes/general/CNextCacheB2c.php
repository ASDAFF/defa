...
if(!class_exists("CNextCacheB2c")){
	class CNextCacheB2c {
				...
				if($arGroupBy === array()){
					...
					if($bGetDetailPageUrlsArray){
						...
						foreach($arByIBlock as $IBLOCK_ID => $arIB){
							...
							if($arSectionIDs){
								$arSections = CNextCacheB2c::CIBlockSection_GetList(array("CACHE" => array("TAG" => CNextCacheB2c::GetIBlockCacheTag($IBLOCK_ID), "MULTI" => "N", "GROUP" => array("ID"))), array("ID" => $arSectionIDs), false, array("ID", "CODE", "EXTERNAL_ID", "IBLOCK_ID"));
							}
							foreach($arIB as $SECTION_ID => $arIDs){
								if($arIDs){
									$rsElements = CIBlockElement::GetList(array(), array("ID" => $arIDs), false, false, array("ID", "DETAIL_PAGE_URL", "CANONICAL_PAGE_URL"));
									$rsElements->SetUrlTemplates(CNextCacheB2c::$arIBlocksInfo[$IBLOCK_ID]["DETAIL_PAGE_URL"]);
		...
		function CSaleBasket_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CNextCacheB2c/sale/CSaleBasket_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		function CCatalogStore_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CNextCacheB2c/catalog/CCatalogStore_GetList/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		function CForumMessage_GetListEx($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $iNum = 0, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CNextCacheB2c/forum/CForumMessage_GetListEx/".$cacheTag."/";
			}
			$obCache = new CPHPCache();
		...
		private function _InitCacheParams($moduleName, $functionName, $arCache){
			...
			if(!strlen($cachePath)){
				$cachePath = "/CNextCacheB2c/".$moduleName."/".$functionName."/".$cacheTag."/";
			}
		...
		function GetIBlockCacheTag($IBLOCK_ID){
			if(!$IBLOCK_ID){
				...
				return @CNextCacheB2c::$arIBlocksInfo[$IBLOCK_ID]["CODE"].$IBLOCK_ID;
			}
		}
		...
		function ClearTagIBlockElement($arFields){
			...
			if($arFields["IBLOCK_ID"]){
				...
				{
					$arBlocks = array();
					if(CNextCacheB2c::$arIBlocks)
					{
						foreach(CNextCacheB2c::$arIBlocks as $arSiteIbloks)
						{
				...
				$CACHE_MANAGER->ClearByTag(CNextCacheB2c::GetIBlockCacheTag($arFields["IBLOCK_ID"]));
		...
		function ClearTagIBlockSectionBeforeDelete($ID){
			global $CACHE_MANAGER;
			if($ID > 0){
				if($IBLOCK_ID = CNextCacheB2c::CIBlockSection_GetList(array("CACHE" => array("MULTI" => "N")), array("ID" => $ID), false, array("IBLOCK_ID"), true)){
					$CACHE_MANAGER->ClearByTag(CNextCacheB2c::GetIBlockCacheTag($IBLOCK_ID));
				}
			}
		}
	}

	// initialize CNextCacheB2c::$arIBlocks array
	if(CNextCacheB2c::$arIBlocks === NULL){
		$arIBlocksTmp = CNextCacheB2c::CIBlock_GetList(array("CACHE" => array("TAG" => "iblocks")), array("ACTIVE" => "Y", "CHECK_PERMISSIONS" => "N"));
		CNextCacheB2c::$arIBlocks = CNextCacheB2c::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("LID", "IBLOCK_TYPE_ID", "CODE"), "MULTI" => "Y", "RESULT" => array("ID")));
		CNextCacheB2c::$arIBlocksInfo = CNextCacheB2c::GroupArrayBy($arIBlocksTmp, array("GROUP" => array("ID")));
	}
}?>