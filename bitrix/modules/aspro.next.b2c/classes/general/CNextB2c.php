...
if(!defined('ASPRO_NEXT_B2C_MODULE_ID')){
	define('ASPRO_NEXT_B2C_MODULE_ID', 'aspro.next.b2c');
}
...
class CNextB2c{
	...
	const solutionName	= 'next.b2c';
	const moduleID		= ASPRO_NEXT_B2C_MODULE_ID;
	const wizardID		= 'aspro:next.b2c';
	const devMode 		= false;
	...
	public static function ShowMobileRegions(){
		...
		if($arRegion):
			...
			$arRegions = CNextRegionalityB2c::getRegions();
			$regionID = ($arRegion ? $arRegion['ID'] : '');
	...
	public static function OnSearchGetURL($arFields)
	{
		...
		{
			$arElement = CNextCacheB2c::CIblockElement_GetList(array('CACHE' => array('TAG' => CNextCacheB2c::GetIBlockCacheTag($arFields['PARAM2']), 'MULTI' => 'N')), array('ID' => $arFields['ITEM_ID']), false, false, array('ID', 'ACTIVE_FROM'));
	...
	public static function GetSections($arItems, $arParams){
		...
		if(is_array($arItems) && $arItems)
		{
			...
			if($arSectionsIDs)
			{
				$arSections['ALL_SECTIONS'] = CNextCacheB2c::CIBLockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CNextCacheB2c::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('ID'), 'MULTI' => 'N')), array('ID' => $arSectionsIDs));
				$bCheckRoot = false;
				foreach($arSections['ALL_SECTIONS'] as $key => $arSection)
				{
					if($arSection['DEPTH_LEVEL'] > 1)
					{
						...
						$arParentSection = CNextCacheB2c::CIBLockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CNextCacheB2c::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'N')), $arFilter, false, $arSelect);
				...
				if($bCheckRoot)
				{
					...
					$arSelect = array('ID', 'SORT', 'IBLOCK_ID', 'NAME');
					$arRootSections = CNextCacheB2c::CIBLockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CNextCacheB2c::GetIBlockCacheTag($arParams['IBLOCK_ID']))), $arFilter, false, $arSelect);
		...
		<?elseif($optionType == "note"):?>
			<?if($optionCode == 'GOALS_NOTE'){
				...
				if(\Bitrix\Main\Loader::includeModule('form'))
				{
					if($optionsSiteID)
					{
						if($arForms = CNextCacheB2c::CForm_GetList($by = array('by' => 's_id', 'CACHE' => array('TAG' => 'forms')), $order = 'asc', array('SITE' => $optionsSiteID, 'SITE_EXACT_MATCH' => 'Y'), $is_filtered))
						{
	...
	public static function Start($siteID){
		...
		if(CModule::IncludeModuleEx(self::moduleID) == 1)
		{
			...
			if($arTheme['USE_REGIONALITY'] == 'Y')
				$arRegion = CNextRegionalityB2c::getCurrentRegion(); //get current region from regionality module
	...
	public static function checkBgImage($siteID){
		...
		if($arBanner === NULL)
		{
			$arFilterBanner = array('IBLOCK_ID' => CNextCacheB2c::$arIBlocks[$siteID]['aspro_next_adv']['aspro_next_bg_images'][0], 'ACTIVE'=>'Y');
			...
			$arItems = CNextCacheB2c::CIBLockElement_GetList(array('SORT' => 'ASC', 'CACHE' => array('TAG' => CNextCacheB2c::GetIBlockCacheTag(CNextCacheB2c::$arIBlocks[$siteID]['aspro_next_adv']['aspro_next_bg_images'][0]))), $arFilterBanner, false, false, array('ID', 'NAME', 'PREVIEW_PICTURE', 'PROPERTY_URL', 'PROPERTY_FIXED_BANNER', 'PROPERTY_URL_NOT_SHOW'));
			$arBanner = array();
			if($arItems)
			{
	...
	public static function ShowPageProps($prop){
		...
		global $APPLICATION;
		$APPLICATION->AddBufferContent(array("CNextB2c", "GetPageProps"), $prop);
	}
		...
		if ($arTheme["ADV_".$position] == 'Y') {
			...
			$APPLICATION->IncludeComponent(
				...
				array(
					...
					"IBLOCK_ID" => CNextCacheB2c::$arIBlocks[SITE_ID]["aspro_next_adv"]["aspro_next_banners_inner"][0],
					"POSITION"	=> $position,
	...
	public static function showPriceMatrix($arItem = array(), $arParams, $strMeasure = '', $arAddToBasketData = array()){
		...
		if(isset($arItem['PRICE_MATRIX']) && $arItem['PRICE_MATRIX'])
		{
			...
			<div class="price_matrix_block">
				<?
				$sDiscountPrices = \Bitrix\Main\Config\Option::get(ASPRO_NEXT_B2C_MODULE_ID, 'DISCOUNT_PRICE');
							...
							<?if($arPrice["PRICE"] > $arPrice["DISCOUNT_PRICE"]){?>
								<div class="price" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["DISCOUNT_PRICE"];?>">
									<?if(strlen($arPrice["DISCOUNT_PRICE"])):?>
										<span class="values_wrapper"><?=\Aspro\Functions\CAsproItemB2c::getCurrentPrice("DISCOUNT_PRICE", $arPrice);?></span><?if(($arParams["SHOW_MEASURE"]=="Y") && $strMeasure && $arPrice["DISCOUNT_PRICE"]):?><span class="price_measure">/<?=$strMeasure?></span><?endif;?>
									<?endif;?>
								</div>
								<?if($arParams["SHOW_OLD_PRICE"]=="Y"):?>
									<div class="price discount" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["PRICE"];?>">
										<span class="values_wrapper"><?=\Aspro\Functions\CAsproItemB2c::getCurrentPrice("PRICE", $arPrice);?></span>
									</div>
								<?endif;?>
							<?}else{?>
								<div class="price" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=$arPrice["DISCOUNT_PRICE"];?>">
									<span><span class="values_wrapper"><?=\Aspro\Functions\CAsproItemB2c::getCurrentPrice("PRICE", $arPrice);?></span><?if(($arParams["SHOW_MEASURE"]=="Y") && $strMeasure && $arPrice["PRICE"]):?><span class="price_measure">/<?=$strMeasure?></span><?endif;?></span>
								</div>
							<?}?>
							<?if($iCountPriceInterval > 1):?>
								</div>
								...
								if($arParams['SHOW_DISCOUNT_PERCENT'] == 'Y' && $arPrice["PRICE"] > $arPrice["DISCOUNT_PRICE"]):?>
									...
									<div class="sale_block">
										...
										<?if($arParams['SHOW_DISCOUNT_PERCENT_NUMBER'] != 'Y'):?>
											<span class="title"><?=GetMessage("CATALOG_ECONOMY");?></span> <div class="text"><span class="values_wrapper" data-currency="<?=$arPrice["CURRENCY"];?>" data-value="<?=($diff*$ratio);?>"><?=\Aspro\Functions\CAsproItemB2c::getCurrentPrice($diff, $arPrice, false)?></span></div>
										<?else:?>
											<?$percent=round(($diff/$arPrice["PRICE"])*100);?>
											<?if($percent && $percent<100){?>
												<div class="value">-<span><?=$percent;?></span>%</div>
											<?}?>
											<div class="text"><?=GetMessage("CATALOG_ECONOMY");?> <span class="values_wrapper"><?=\Aspro\Functions\CAsproItemB2c::getCurrentPrice($diff, $arPrice, false);?></span></div>
										<?endif;?>
									</div>
								<?endif;?>
							<?endif;?>
			...
			foreach(GetModuleEvents(ASPRO_NEXT_B2C_MODULE_ID, 'OnAsproShowPriceMatrix', true) as $arEvent) // event for manipulation price matrix
		...
		if($arItem)
		{
			if(isset($arItem['PRICE_MATRIX']) && $arItem['PRICE_MATRIX'])
			{
				...
				<div class="with_matrix">
					<div class="price">
						<div class="price_value_block"><?=\Aspro\Functions\CAsproItemB2c::getCurrentPrice("PRICE", $arItem['ITEM_PRICES'][$priceSelected]);?></div><?if (($arParams['SHOW_MEASURE'] == 'Y') && $arItem['CATALOG_MEASURE_NAME'] && $arItem['ITEM_PRICES'][$priceSelected]["PRICE"]):?><span class="price_measure">/<?=$arItem['CATALOG_MEASURE_NAME']?></span><?endif;?>
					</div>
					<?if($arParams['SHOW_OLD_PRICE'] == 'Y' && $arItem['ITEM_PRICES'][$priceSelected]['BASE_PRICE'] > $arItem['ITEM_PRICES'][$priceSelected]['PRICE']):?>
						<div class="price discount"><?=\Aspro\Functions\CAsproItemB2c::getCurrentPrice("BASE_PRICE", $arItem['ITEM_PRICES'][$priceSelected]);?></div>
					<?endif;?>
						...
						<div class="sale_block matrix">
							<div class="sale_wrapper">
								<?if($arParams['SHOW_DISCOUNT_PERCENT_NUMBER'] != 'Y'):?>
									...
									<div class="text">
										<span class="values_wrapper"><?=\Aspro\Functions\CAsproItemB2c::getCurrentPrice("DISCOUNT", $arItem['ITEM_PRICES'][$priceSelected]);?></span></div>
								<?else:?>
				...
				<?$html = ob_get_contents();
				ob_end_clean();
				foreach(GetModuleEvents(ASPRO_NEXT_B2C_MODULE_ID, 'OnAsproShowPriceRangeTop', true) as $arEvent) // event for manipulation price matrix top
	...
	public static function GetTotalCount($arItem, $arParams = array()){
		...
		foreach(GetModuleEvents(ASPRO_NEXT_B2C_MODULE_ID, 'OnAsproGetTotalQuantity', true) as $arEvent) // event for manipulation total quantity
	...
	public static function GetQuantityArray($totalCount, $arItemIDs = array(), $useStoreClick="N"){
		...
		foreach(GetModuleEvents(ASPRO_NEXT_B2C_MODULE_ID, 'OnAsproGetTotalQuantityBlock', true) as $arEvent) // event for manipulation store quantity block
	...
	public static function GetSKUPropsArray(&$arSkuProps, $iblock_id=0, $type_view="list", $hide_title_props="N", $group_iblock_id="N", $arItem = array()){
		...
		if($iblock_id){
			//$arPropsSku=CNextB2c::GetPropertyViewType($iblock_id);
			...
			foreach ($arSkuProps as $key=>&$arProp){
				...
				if(($arProp["DISPLAY_TYPE"]=="P" || $arProp["DISPLAY_TYPE"]=="R" ) && $type_view!= 'block' ){
					...
					if($isHasPicture)
					{
						$templateRow .= '<div class="bx_item_detail_scu" id="#ITEM#_prop_'.$arProp['ID'].'_cont" data-display_type="LI" data-id="'.$arProp['ID'].'">'.
	'<span class="'.$class_title.'">'.$hint_block.'<span>'.htmlspecialcharsex($arProp['NAME']).'</span></span>'.
	'<div class="bx_scu_scroller_container"><div class="bx_scu 111"><ul id="#ITEM#_prop_'.$arProp['ID'].'_list" class="list_values_wrapper">';
					}
					else
					{
						$templateRow .= '<div class="bx_item_detail_size" id="#ITEM#_prop_'.$arProp['ID'].'_cont" data-display_type="LI" data-id="'.$arProp['ID'].'">'.
					...
					$templateRow .= '</ul></div>'.
	'</div></div>';
					if($isHasPicture) {//if has colors, then add a form feedback 'I need a more colors'
						$templateRow .= '<div class="cheaper_form"><span class="animate-load" data-event="jqm" data-param-form_id="SIMPLE_FORM_11" data-name="cheaper" data-autoload-product_name="'.self::formatJsName($arProp['NAME']).'" data-autoload-product_id="'.$arProp['ID'].'">Нужен в другом цвете?</span></div>';
					}
				}
	...
	public static function GetAddToBasketArray(&$arItem, $totalCount = 0, $defaultCount = 1, $basketUrl = '', $bDetail = false, $arItemIDs = array(), $class_btn = "small", $arParams=array()){
		...
		foreach(GetModuleEvents(ASPRO_NEXT_B2C_MODULE_ID, 'OnAsproGetBuyBlockElement', true) as $arEvent) // event for manipulation with buy block element
	...
	public static function checkBreadcrumbsChain(&$arParams, $arSection = array(), $arElement = array()){
		...
		if(Option::get(self::moduleID, "SHOW_BREADCRUMBS_CATALOG_CHAIN", "H1", SITE_ID) == "NAME"){
			...
			if($arParams['INCLUDE_IBLOCK_INTO_CHAIN'] == 'Y' && isset(CNextCacheB2c::$arIBlocksInfo[$arParams['IBLOCK_ID']]['NAME'])){
				$APPLICATION->AddChainItem(CNextCacheB2c::$arIBlocksInfo[$arParams['IBLOCK_ID']]['NAME'], $arElement['~LIST_PAGE_URL']);
			}
	...
	public static function GetItemsYear($arParams){
		$arResult = array();
		$arItems = CNextCacheB2c::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CNextCacheB2c::GetIBlockCacheTag($arParams['IBLOCK_ID']))), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'ACTIVE' => 'Y'), false, false, array('ID', 'NAME', 'ACTIVE_FROM'));
		if($arItems)
		{
			foreach($arItems as $arItem)
			{
	...
	public static function SetMeta(){
		...
		if($arRegion)
		{
			...
			if($arTagSeoMarks)
				CNextRegionalityB2c::addSeoMarks($arTagSeoMarks);
		}
	}
	...
	public static function setFooterTitle(){
		...
		if($arRegion)
		{
			CNextRegionalityB2c::addSeoMarks(array('#REGION_ID#' => 'ID'));
			CNextRegionalityB2c::replaceSeoMarks();
		}
		else
		{
	...
	public static function getBasketItems($iblockID=0, $field="PRODUCT_ID"){
		...
		if(\Bitrix\Main\Loader::includeModule("sale"))
		{
			...
			{
				$compare_items = array();
				$iblockID=((isset($iblockID) && $iblockID) ? $iblockID : \Bitrix\Main\Config\Option::get(self::moduleID, "CATALOG_IBLOCK_ID", CNextCacheB2c::$arIBlocks[SITE_ID]['aspro_next_catalog']['aspro_next_catalog'][0], SITE_ID ));
