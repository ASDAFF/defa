...
class CNextEventsB2c{
	...
	public static function OnBeforeEventAddHandler(&$event, &$lid, &$arFields, &$message_id){
		...
		{
			if($arCurrentRegion = CNextRegionalityB2c::getCurrentRegion())
			{
				...
				foreach(CNextRegionalityB2c::$arSeoMarks as $mark => $field)
				{
			...
			if($arPhoneProp && $arParams['USE_PHONE_NORMALIZATION'] !='N')
			{
				...
				if($arUserResult['ORDER_PROP'][$arPhoneProp['ID']])
				{
					...
					else
					{
						...
						{
							$mask = \Bitrix\Main\Config\Option::get('aspro.next.b2c', 'PHONE_MASK', '+7 (999) 999-99-99');
	...
	function OnBeforeUserUpdateHandler(&$arFields){
		...
		if($bAdminSection)
	    {
			...
			$sOneFIO = COption::GetOptionString(ASPRO_NEXT_B2C_MODULE_ID, 'PERSONAL_ONEFIO', 'Y', $siteID);
			$sChangeLogin = COption::GetOptionString(ASPRO_NEXT_B2C_MODULE_ID, 'LOGIN_EQUAL_EMAIL', 'Y', $siteID);
        }
		else
		{
			$arFrontParametrs = CNextB2c::GetFrontParametrsValues($siteID);
	...
	static function clearBasketCacheHandler($orderID, $arFields, $arParams = array()){
		...
		if(isset($arFields) && $arFields)
		{
			if(isset($arFields["ID"]) && $arFields["ID"])
			{
				...
				$arUser = $arUser = CNextCacheB2c::CUser_GetList(array("SORT" => "ASC", "CACHE" => array("MULTI" => "N", "TAG" => CNextCacheB2c::GetUserCacheTag($USER_ID))), array("ID" => $USER_ID), array("FIELDS" => array("ID", "PERSONAL_PHONE")));
				...
				if(self::isLandingSearchIblock($IBLOCK_ID)){
					...
					while($arSeoSearchElementQuery = $dbRes->Fetch()){
						if(strlen($query = trim($arSeoSearchElementQuery['VALUE']))){
							list($query, $hash, $arData) = \Aspro\Next\SearchQueryB2c::getSentenceMeta($query);
					...
					if($arLandingSearchQuery){
						if(strlen($queryExample = \Aspro\Next\SearchQueryB2c::getSentenceExampleQuery(reset($arLandingSearchQuery), LANG))){
					...
					if(CNextCacheB2c::$arIBlocksInfo[$IBLOCK_ID]){
						$arSitesLids = CNextCacheB2c::$arIBlocksInfo[$IBLOCK_ID]['LID'];
									...
									if($arSite){
										...
										if(!$condIblockId){
											$condIblockId = \Bitrix\Main\Config\Option::get(
												self::moduleID,
												'CATALOG_IBLOCK_ID',
												CNextCacheB2c::$arIBlocks[$siteId]['aspro_next_catalog']['aspro_next_catalog'][0],
												$siteId
											);
										}
										
										if(isset(CNextCacheB2c::$arIBlocksInfo[$condIblockId])){
											$pathFile = str_replace(array('#SITE_DIR#', 'index.php'), array($siteDir, ''), CNextCacheB2c::$arIBlocksInfo[$condIblockId]['LIST_PAGE_URL']).'index.php';
	...
	static public function isLandingSearchIblock($IBLOCK_ID){
		return isset(CNextCacheB2c::$arIBlocksInfo[$IBLOCK_ID]) && strpos(CNextCacheB2c::$arIBlocksInfo[$IBLOCK_ID]['CODE'], 'aspro_next_search') !== false;
	}
	...
	static function OnEndBufferContentHandler(&$content)
	{
		if(!defined('ADMIN_SECTION') && !defined('WIZARD_SITE_ID'))
		{
				...
				if($start>0)
				{
					...
					if(($end>0) && ($end>$start))
					{
						if(defined("BX_UTF") && BX_UTF === true)
							$content = CNextB2c::utf8_substr_replace($content, "", $start, $end-$start);
						else
			...
			//process recaptcha
			if(\Aspro\Functions\CAsproNextReCaptchaB2c::checkRecaptchaActive())
			{
				$count = 0;
				$contentReplace = preg_replace_callback(
					...
					function ($arImage)
					{
						...
						return \Aspro\Functions\CAsproNextReCaptchaB2c::callbackReplaceImage($arImage);
					},
					$html,
					-1,
					$count
				...
				$captcha_public_key = \Aspro\Functions\CAsproNextReCaptchaB2c::getPublicKey();
	...
	public static function OnPageStartHandler(){
		...
		{
			if($code = \Aspro\Functions\CAsproNextReCaptchaB2c::getCodeByPostList($arPostData))
			{
		...
		foreach($arPostData as $key => $arPost)
		{
			...
			if($code = \Aspro\Functions\CAsproNextReCaptchaB2c::getCodeByPostList($arPost))
			{
		...
		if($needReInit)
		{
			\Aspro\Functions\CAsproNextReCaptchaB2c::reInitContext($application, $request);
		}
	}
	...
	static function OnIBlockPropertyBuildListStoresHandler(){
		return array(
			...
			"GetPropertyFieldHtml"   => array("CNextEventsB2c", "GetPropertyFieldHtmlStoresHandler"),
			"GetPropertyFieldHtmlMulty"   => array("CNextEventsB2c", "GetPropertyFieldHtmlStoresHandlerMulty"),
		);
	}
	...
	static function OnIBlockPropertyBuildListLocationsHandler(){
		return array(
			...
			"GetPropertyFieldHtml"   => array("CNextEventsB2c", "GetPropertyFieldHtmlLocationsHandler"),
		);
	}
	...
	static function OnIBlockPropertyBuildListPricesHandler(){
		return array(
			...
			"GetPropertyFieldHtml"   => array("CNextEventsB2c", "GetPropertyFieldHtmlPricesHandler"),
			"GetPropertyFieldHtmlMulty"   => array("CNextEventsB2c", "GetPropertyFieldHtmlPricesHandlerMulty"),
		);
	}
	...
	static function OnIBlockPropertyBuildCustomFilterHandler(){
		return array(
			...
			'GetPropertyFieldHtml'   => array('CNextEventsB2c', 'GetPropertyFieldHtmlCustomFilterHandler'),
			'GetSettingsHTML'   => array('CNextEventsB2c', 'GetSettingsHTMLCustomFilterHandler'),
			'PrepareSettings'   => array('CNextEventsB2c', 'PrepareSettingsCustomFilterHandler'),
		);
	}
	...
	static function OnRegionUpdateHandler($arFields){
		...
		if(isset(CNextCacheB2c::$arIBlocks[$arIBlock['LID']]['aspro_next_regionality']['aspro_next_regions'][0]) && CNextCacheB2c::$arIBlocks[$arIBlock['LID']]['aspro_next_regionality']['aspro_next_regions'][0])
			$iRegionIBlockID = CNextCacheB2c::$arIBlocks[$arIBlock['LID']]['aspro_next_regionality']['aspro_next_regions'][0];
		else
			return;
	...
	static function onBeforeResultAddHandler($WEB_FORM_ID, &$arFields, &$arrVALUES){
		...
		{
			global $APPLICATION;
			$arTheme = CNextB2c::GetFrontParametrsValues(SITE_ID);
