...
if(!class_exists('CNextRegionalityB2c'))
{
	class CNextRegionalityB2c{
		...
		public static function checkUseRegionality(){
			if(\Bitrix\Main\Loader::includeModule(ASPRO_NEXT_B2C_MODULE_ID))
			{
				return CNextB2c::GetFrontParametrValue('USE_REGIONALITY');
			}
			return 'N';
		}
		...
		public static function getRegionIBlockID(){
			...
			{
				if(isset(CNextCacheB2c::$arIBlocks[SITE_ID]['aspro_next_regionality']['aspro_next_regions'][0]) && CNextCacheB2c::$arIBlocks[SITE_ID]['aspro_next_regionality']['aspro_next_regions'][0])
				{
					$iRegionIBlockID = CNextCacheB2c::$arIBlocks[SITE_ID]['aspro_next_regionality']['aspro_next_regions'][0];
				}
				else
				{
					return;
		...
		public static function replaceSeoMarks(){
			...
			if($arRegion && $page_title)
			{
				...
				{
					$bShowSiteName = (\Bitrix\Main\Config\Option::get(ASPRO_NEXT_B2C_MODULE_ID, "HIDE_SITE_NAME_TITLE", "N") == "N");
			...
			if($arRegions === NULL)
			{
				...
				if($iRegionIBlockID = self::getRegionIBlockID())
				{
					...
					else
					{
						...
						foreach(GetModuleEvents(ASPRO_NEXT_B2C_MODULE_ID, 'OnAsproRegionalityAddSelectFieldsAndProps', true) as $arEvent) // event for add to select in region getlist elements
							ExecuteModuleEventEx($arEvent, array(&$arSelect));

						//$arItems = CNextCacheB2c::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => CNextCacheB2c::GetIBlockCacheTag($iRegionIBlockID), 'GROUP' => 'ID', 'CAN_MULTI_SECTION' => 'N')), $arFilter, false, false, $arSelect);
						...
						foreach(GetModuleEvents(ASPRO_NEXT_B2C_MODULE_ID, 'OnAsproRegionalityGetElements', true) as $arEvent) // event for manipulation with region elements
