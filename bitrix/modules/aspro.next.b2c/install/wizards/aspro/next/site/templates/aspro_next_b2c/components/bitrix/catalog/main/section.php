...
if($arResult["VARIABLES"]["SECTION_ID"] > 0){
	$section=CNextCacheB2c::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arResult["VARIABLES"]["SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "DESCRIPTION", "UF_SECTION_DESCR", "UF_OFFERS_TYPE", $arParams["SECTION_DISPLAY_PROPERTY"], "IBLOCK_SECTION_ID", "DEPTH_LEVEL", "LEFT_MARGIN", "RIGHT_MARGIN"));
}
elseif(strlen(trim($arResult["VARIABLES"]["SECTION_CODE"])) > 0){

	$section=CNextCacheB2c::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "=CODE" => $arResult["VARIABLES"]["SECTION_CODE"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "DESCRIPTION", "UF_SECTION_DESCR", "UF_OFFERS_TYPE", $arParams["SECTION_DISPLAY_PROPERTY"], "IBLOCK_SECTION_ID", "DEPTH_LEVEL", "LEFT_MARGIN", "RIGHT_MARGIN"));
}
...
if($section){
	...
	$posSectionDescr = COption::GetOptionString("aspro.next.b2c", "SHOW_SECTION_DESCRIPTION", "BOTTOM", SITE_ID);

	$iSectionsCount = CNextCacheB2c::CIBlockSection_GetCount(array('CACHE' => array("TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array("SECTION_ID" => $arSection["ID"], "ACTIVE" => "Y", "GLOBAL_ACTIVE" => "Y"));
	...
	$itemsCnt = CNextCacheB2c::CIBlockElement_GetList(array("CACHE" => array("TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), $arElementFilter, array());
	...
	else
	{
		...
		{
			$sectionParent = CNextCacheB2c::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $section["IBLOCK_SECTION_ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "UF_OFFERS_TYPE"));
			if($sectionParent['UF_OFFERS_TYPE'] && !$typeTmpSKU)
				$typeTmpSKU = $sectionParent['UF_OFFERS_TYPE'];

			if(!$typeTmpSKU)
			{
				$sectionRoot = CNextCacheB2c::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "<=LEFT_BORDER" => $section["LEFT_MARGIN"], ">=RIGHT_BORDER" => $section["RIGHT_MARGIN"], "DEPTH_LEVEL" => 1, "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "UF_OFFERS_TYPE"));
...
<?
//seo
$arSeoItems = CNextCacheB2c::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"Y", "TAG" => CNextCacheB2c::GetIBlockCacheTag(CNextCacheB2c::$arIBlocks[SITE_ID]["aspro_next_catalog"]["aspro_next_catalog_info"][0]))), array("IBLOCK_ID" => CNextCacheB2c::$arIBlocks[SITE_ID]["aspro_next_catalog"]["aspro_next_catalog_info"][0], "ACTIVE"=>"Y"), false, false, array("ID", "IBLOCK_ID", "NAME", "PREVIEW_TEXT", "DETAIL_PICTURE", "PROPERTY_FILTER_URL", "PROPERTY_LINK_REGION", "PROPERTY_FORM_QUESTION", "PROPERTY_SECTION_SERVICES", "PROPERTY_TIZERS", "PROPERTY_SECTION", "DETAIL_TEXT", "ElementValues"));
...
if(CNextB2c::GetFrontParametrValue('CATALOG_COMPARE') == 'N')
...
<?CNextB2c::checkBreadcrumbsChain($arParams, $arSection);?>
