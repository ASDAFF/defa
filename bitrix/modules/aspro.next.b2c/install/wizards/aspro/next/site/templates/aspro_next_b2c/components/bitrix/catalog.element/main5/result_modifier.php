...
$productSlider = CNextB2c::getSliderForItemExt($arResult, $arParams['ADD_PICT_PROP'], 'Y' == $arParams['ADD_DETAIL_TO_SLIDER']);
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		...
		$offerSlider = CNextB2c::getSliderForItemExt($arOffer, $arParams['OFFER_ADD_PICT_PROP'], true); // $arParams['ADD_DETAIL_TO_SLIDER'] == 'Y'
	...
	$postfix = '';
	global $arSite;
	if(\Bitrix\Main\Config\Option::get("aspro.next.b2c", "HIDE_SITE_NAME_TITLE", "N")=="N")
	...
	if('TYPE_1' == $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
		{
			...
			$totalCount = CNextB2c::GetTotalCount($arOffer, $arParams);
			$arOffer['IS_OFFER'] = 'Y';
			...
			if($arParams['USE_PRICE_COUNT'] == 'Y')
			{
				...
				$arOffer = array_merge($arOffer, CNextB2c::formatPriceMatrix($arOffer));
				$sPriceMatrix = CNextB2c::showPriceMatrix($arOffer, $arParams, $arOffer['~CATALOG_MEASURE_NAME']);
			}

			$arAddToBasketData = CNextB2c::GetAddToBasketArray($arOffer, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'btn-lg w_icons', $arParams);
			...
			$arOneRow = array(
				...
				'AVAILIABLE' => CNextB2c::GetQuantityArray($totalCount, array(), "Y"),
	...
	/*set min_price_id*/
	if('TYPE_1' != $arParams['TYPE_SKU'] && $arResult['OFFERS'] ){
		$arResult['MIN_PRICE'] = CNextB2c::getMinPriceFromOffersExt(
			$arResult['OFFERS'],
		...
		foreach ($arResult['OFFERS'] as $keyOffer => $arOffer){
			...
			if($arParams['USE_PRICE_COUNT'] == 'Y')
			{
				...
				$arResult['OFFERS'][$keyOffer] = array_merge($arOffer, CNextB2c::formatPriceMatrix($arOffer));
			}
		}
	...
	if($arParams['USE_PRICE_COUNT'] == 'Y')
	{
		if($arResult['OFFERS'])
		{
			foreach($arResult['OFFERS'] as $keyOffer => $arOffer)
			{
				...
				$arOffer["FIX_PRICE_MATRIX"] = CNextB2c::checkPriceRangeExt($arOffer);
				$arResult['OFFERS'][$keyOffer] = array_merge($arOffer, CNextB2c::formatPriceMatrix($arOffer));
			}
			$arResult['MIN_PRICE'] = CNextB2c::getMinPriceFromOffersExt(
				$arResult['OFFERS'],
		...
		{
			$arResult["FIX_PRICE_MATRIX"] = CNextB2c::checkPriceRangeExt($arResult);
		}
	}

	//format prices when USE_PRICE_COUNT
	$arResult = array_merge($arResult, CNextB2c::formatPriceMatrix($arResult));
}
...
if (intVal($arParams["IBLOCK_STOCK_ID"])){
	...
	$arResult["STOCK"]=CNextCacheB2c::CIBLockElement_GetList(array('CACHE' => array("TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_STOCK_ID"]))), array("IBLOCK_ID" => $arParams["IBLOCK_STOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "PROPERTY_LINK_GOODS" => $arResult["ID"]), false, false, $arSelect);
}

/*services*/
if( !empty($arResult["PROPERTIES"]["SERVICES"]["VALUE"]) ){
	$arSelect = array("ID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "DETAIL_PAGE_URL");
	$arResult["SERVICES"]=CNextCacheB2c::CIBLockElement_GetList(array('CACHE' => array("TAG" => CNextCacheB2c::GetIBlockCacheTag($arResult["PROPERTIES"]["SERVICES"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arResult["PROPERTIES"]["SERVICES"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "ACTIVE_DATE" => "Y", "ID" => $arResult["PROPERTIES"]["SERVICES"]["VALUE"]), false, false, $arSelect);
}

/*brand item*/
$arBrand = array();
if(strlen($arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]) && $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]){
	$arBrand = CNextCacheB2c::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCacheB2c::GetIBlockCacheTag($arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"]))), array("IBLOCK_ID" => $arResult["PROPERTIES"]["BRAND"]["LINK_IBLOCK_ID"], "ACTIVE"=>"Y", "ID" => $arResult["DISPLAY_PROPERTIES"]["BRAND"]["VALUE"]), false, false, array("ID", "NAME", "DETAIL_PAGE_URL", "PREVIEW_PICTURE", "PREVIEW_TEXT"));
	if($arBrand){
...
/*stores product*/
$arStores=CNextCacheB2c::CCatalogStore_GetList(array(), array("ACTIVE" => "Y"), false, false, array());
...
if($arResult["SECTION"])
{
	$arSection = CNextCacheB2c::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "ID" => $arResult["SECTION"]["ID"], "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "LEFT_MARGIN", "RIGHT_MARGIN", "NAME", "UF_TABLE_SIZES"));
	...
	if(!$tmpTableSizes)
	{
		...
		{
			$sectionRoot = CNextCacheB2c::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array('GLOBAL_ACTIVE' => 'Y', "<=LEFT_BORDER" => $arSection["LEFT_MARGIN"], ">=RIGHT_BORDER" => $arSection["RIGHT_MARGIN"], "DEPTH_LEVEL" => 1, "IBLOCK_ID" => $arParams["IBLOCK_ID"]), false, array("ID", "IBLOCK_ID", "NAME", "UF_TABLE_SIZES"));
