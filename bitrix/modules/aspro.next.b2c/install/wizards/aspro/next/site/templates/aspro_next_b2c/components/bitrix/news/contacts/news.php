...
$arItemFilter = CNextB2c::GetIBlockAllElementsFilter($arParams);
$arItemSelect = array('ID', 'NAME', 'IBLOCK_ID', 'IBLOCK_SECTION_ID', 'PROPERTY_MAP');
$arItems = CNextCacheB2c::CIblockElement_GetList(array("CACHE" => array("TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), $arItemFilter, false, false, $arItemSelect);

$arAllSections = array();
if($arItems)
	$arAllSections = CNextB2c::GetSections($arItems, $arParams);
?>
				...
				<div class="col-md-<?=($bHasSections ? 6 : 12);?>">
					<div class="row">
						<div class="col-md-6 print-6">
							<?CNextB2c::showContactPhones('', true, '', 'Phone_black2.svg', 'grey');?>
						</div>
						<div class="col-md-6 print-6">
							<?CNextB2c::showContactEmail('E-mail', true, '', 'Email.svg', 'grey');?>
						</div>
					</div>
...
<div class="ajax_items">
	...
	<?if($arItems):?>
		...
		<?
		$bUseMap = CNextB2c::GetFrontParametrValue('CONTACTS_USE_MAP', SITE_ID) != 'N';
		$mapLAT = $mapLON = $iCountShops =0;
		$arPlacemarks = array();
		if($bPostSection)
		{
			$arItems = CNextCacheB2c::CIblockElement_GetList(array("CACHE" => array("TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array_merge($arItemFilter, array('SECTION_ID' => $_POST['ID'])), false, false, $arItemSelect);
