...
<div class="menu middle">
	<ul>
		<li<?=(CNextB2c::isPersonalPage() ? ' class="selected"' : '')?>>
			<?$link = str_replace('#SITE_DIR#', SITE_DIR, \Bitrix\Main\Config\Option::get('aspro.next.b2c', 'PERSONAL_PAGE_URL', SITE_DIR.'personal/'));?>
			<a rel="nofollow" class="dark-color<?=($bParent ? ' parent' : '')?>" href="<?=$link;?>">
				<?=CNextB2c::showIconSvg("cabinet", SITE_TEMPLATE_PATH."/images/svg/".($USER->IsAuthorized() ? 'User' : 'Lock')."_black.svg");?>
