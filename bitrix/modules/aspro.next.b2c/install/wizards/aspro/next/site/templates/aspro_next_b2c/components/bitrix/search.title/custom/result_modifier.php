...
if (!empty($arResult["ELEMENTS"]) && CModule::IncludeModule("iblock"))
{
	...
	$bHideNotAvailable = (\Bitrix\Main\Config\Option::get('aspro.next.b2c', 'SEARCH_HIDE_NOT_AVAILABLE', 'N') == 'Y');
	...
	while($arElement = $rsElements->Fetch())
	{
		...
		if($arOffers){
			...
			$arResult["ELEMENTS"][$arElement["ID"]]["MIN_PRICE"]=CNextB2c::getMinPriceFromOffersExt(
					$arOffers,
