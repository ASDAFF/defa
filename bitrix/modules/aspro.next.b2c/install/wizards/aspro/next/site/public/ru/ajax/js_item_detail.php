...
<?if($arPost["PARAMS"]):?>
	...
	{
		$arPricesIDs = \Aspro\Functions\CAsproNextB2c::getPricesID($arPost["PARAMS"]["PRICE_CODE"], true);
		if($arPricesIDs)
		{
	...
	$cachePath = "/CNextCacheB2c/iblock/getSKUjs/".$cacheTag."/";
	$cacheTime = $arPost["PARAMS"]["CACHE_TIME"];
	// $cacheTime = 0;

	if(isset($arPost["clear_cache"]) && $arPost["clear_cache"] == "y")
		\CNextCacheB2c::ClearCacheByTag($cacheTag);
	...
	else
	{
			...
			if($arOffer['PRICES'])
			{
				...
				if($arPost["PARAMS"]["USE_PRICE_COUNT"] == "Y")
				{
					if(function_exists('CatalogGetPriceTableEx'))
					{
						...
						{
							$arOffer = array_merge($arResult["ITEMS"][$key], CNextB2c::formatPriceMatrix($arResult["ITEMS"][$key]));
		...
		foreach($arResult["ITEMS"] as $key => $arItem)
		{
			$arItems["ITEMS"][$key] = array(
				...
				"TOTAL_COUNT" => CNextB2c::GetTotalCount($arItem, $arPost["PARAMS"])
			);
		}
	...
	if($arItems)
	{
		foreach($arItems["ITEMS"] as $key => $arItem)
		{
			...
			$arAddToBasketData = CNextB2c::GetAddToBasketArray($arItem, $arItem["TOTAL_COUNT"], $arPost["PARAMS"]["DEFAULT_COUNT"], $arPost["PARAMS"]["BASKET_URL"], false, array(), 'small read_more1', $arPost["PARAMS"]);
			...
			$arItems["ITEMS"][$key]["AVAILIABLE"] = CNextB2c::GetQuantityArray($arItem["TOTAL_COUNT"]);
			...
			{
				$arItems["ITEMS"][$key]["PRICE_MATRIX_HTML"] = CNextB2c::showPriceMatrix($arItem, $arPost["PARAMS"], $arItem['MEASURE']);
