				...
				<ul class="tabs_slider slides">
					...
					<?foreach($arResult['ITEMS'] as $key => $arItem){
						...
						if(!$arItem["OFFERS"]){
							...
							$arAddToBasketData = CNextB2c::GetAddToBasketArray($arItem, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'small', $arParams);
						}
						elseif($arItem["OFFERS"]){
							$strMeasure = $arItem["MIN_PRICE"]["CATALOG_MEASURE_NAME"];
							if(!$arItem['OFFERS_PROP']){

								$arAddToBasketData = CNextB2c::GetAddToBasketArray($arItem["OFFERS"][0], $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'small', $arParams);
							}
						}
							...
							<div class="inner_wrap">
								...
								<div class="item_info <?=$arParams["TYPE_SKU"]?>">
									...
									<div class="cost prices clearfix">
										<?if( $arItem["OFFERS"]){?>
											<?\Aspro\Functions\CAsproSkuB2c::showItemPrices($arParams, $arItem, $item_id, $min_price_id, $arItemIDs);?>
										<?}elseif ( $arItem["PRICES"] ){?>
											...
											<?\Aspro\Functions\CAsproItemB2c::showItemPrices($arParams, $arItem["PRICES"], $strMeasure, $min_price_id);?>
										<?}?>
									</div>
										...
										<?if(!empty($arItem['OFFERS_PROP'])){?>
											<div class="sku_props">
												<div class="bx_catalog_item_scu wrapper_sku" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PROP_DIV']; ?>">
													<?$arSkuTemplate = array();?>
													<?$arSkuTemplate=CNextB2c::GetSKUPropsArray($arItem['OFFERS_PROPS_JS'], $arResult["SKU_IBLOCK_ID"], $arParams["DISPLAY_TYPE"], $arParams["OFFER_HIDE_NAME_PROPS"]);?>
												...
												<?$arItemJSParams=CNextB2c::GetSKUJSParams($arResult, $arParams, $arItem);?>
								...
								<?elseif($arItem["OFFERS"]):?>
									<?if(empty($arItem['OFFERS_PROP'])){?>
										<div class="offer_buy_block buys_wrapp woffers">
											...
											$arAddToBasketData = CNextB2c::GetAddToBasketArray($arItem, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, $arItemIDs["ALL_ITEM_IDS"], 'small read_more1', $arParams);?>
											<!--noindex-->
												<?=$arAddToBasketData["HTML"]?>
											<!--/noindex-->
										</div>
