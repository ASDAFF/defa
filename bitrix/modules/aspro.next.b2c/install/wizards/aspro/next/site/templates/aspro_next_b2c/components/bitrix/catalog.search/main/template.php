...
$oSearchQuery = new \Aspro\Next\SearchQueryB2c($searchQuery);
...
if($arLanding){
	...
	if($arLanding['PROPERTY_CUSTOM_FILTER_VALUE'] && $arLanding['PROPERTY_CUSTOM_FILTER_TYPE_VALUE']){
		...
		if(\Bitrix\Main\Loader::includeModule('catalog') && class_exists('CNextConditionB2c')){
			$arCustomFilter = array();
			$cond = new CNextConditionB2c();
	...
	if($bReplaceElementsByCustomFilter = $arCustomFilter && $arLanding['PROPERTY_CUSTOM_FILTER_TYPE_VALUE'] && strpos($arLanding['PROPERTY_CUSTOM_FILTER_TYPE_VALUE'], GetMessage('REPLACE_SUBSTR')) !== false){
		...
		$arElements = CNextCacheB2c::CIBLockElement_GetList(
			array(
				'CACHE' => array(
					'MULTI' => 'Y',
		...
		<?if($arLanding["PROPERTY_TIZERS_VALUE"]):?>
			...
			<?$APPLICATION->IncludeComponent(
				...
				array(
					...
					"IBLOCK_ID" => CNextCacheB2c::$arIBlocks[SITE_ID]["aspro_next_content"]["aspro_next_tizers"][0],
					"NEWS_COUNT" => "4",
					"SORT_BY1" => "SORT",
					"SORT_ORDER1" => "ASC",
					"SORT_BY2" => "ID",
...
if (is_array($arElements) && !empty($arElements))
{
	...
	$arItems = CNextCacheB2c::CIBLockElement_GetList(
		array(
			'CACHE' => array("MULTI" =>"Y", "TAG" => CNextCacheB2c::GetIBlockCacheTag($catalogIBlockID))
		),
		$arItemsFilter,
		false,
		false,
	...
	if($arItems)
	{
		...
		if($arAllSections)
		{
			...
			$arSections = CNextCacheB2c::CIBlockSection_GetList(array('CACHE' => array("MULTI" =>"N", "GROUP" => "ID", "TAG" => CNextCacheB2c::GetIBlockCacheTag($catalogIBlockID))), array("ID" => $arSectionsID, "IBLOCK_ID" => $catalogIBlockID), false, array("ID", "IBLOCK_ID", "NAME"));
		}?>
	...
	<?if($arLanding['PROPERTY_SIMILAR_VALUE']):?>
		...
		<?$APPLICATION->IncludeComponent(
			...
			array(
				...
				"IBLOCK_ID" => CNextCacheB2c::$arIBlocks[SITE_ID]["aspro_next_catalog"]["aspro_next_search"][0],
				"NEWS_COUNT" => "999",
