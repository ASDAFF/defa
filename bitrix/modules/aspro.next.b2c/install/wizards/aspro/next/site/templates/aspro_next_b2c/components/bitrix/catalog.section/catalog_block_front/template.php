...
<?if( count( $arResult["ITEMS"] ) >= 1 ){?>
	<div class="top_wrapper items_wrapper">
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
			...
			$arQuantityData = CNextB2c::GetQuantityArray($totalCount);
			...
			$arAddToBasketData = CNextB2c::GetAddToBasketArray($arItem, $totalCount, $arParams["DEFAULT_COUNT"], $arParams["BASKET_URL"], false, array(), 'small', $arParams);
					...
					<div class="inner_wrap">
							...
							<?if($arParams["DISPLAY_WISH_BUTTONS"] != "N" || $arParams["DISPLAY_COMPARE"] == "Y"):?>
								<div class="like_icons">
									<?if($arParams["DISPLAY_WISH_BUTTONS"] == "Y" && !$arItem["OFFERS"]):?>
										<div class="wish_item_button" <?=(CNextB2c::checkShowDelay($arParams, $totalCount, $arItem) ? '' : 'style="display:none"');?>>
						...
						<div class="item_info">
							...
							<div class="cost prices clearfix">
								<?if( $arItem["OFFERS"]){?>
									<?\Aspro\Functions\CAsproSkuB2c::showItemPrices($arParams, $arItem, $item_id, $min_price_id, array(), ($arParams["SHOW_DISCOUNT_PERCENT_NUMBER"] == "Y" ? "N" : "Y"));?>
								<?}else{?>
									<?
									$item_id = $arItem["ID"];
						...
						<div class="footer_button">
							<div class="counter_wrapp">
								<div class="button_block">
									<!--noindex-->
										<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="btn btn-default basket read_more"><?=\Bitrix\Main\Config\Option::get('aspro.next.b2c', "EXPRESSION_READ_MORE_OFFERS_DEFAULT", GetMessage("CATALOG_READ_MORE"));?></a>
									<!--/noindex-->
								</div>
							</div>
						</div>
