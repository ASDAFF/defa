...
$arSection = CNextCacheB2c::CIblockSection_GetList(array("CACHE" => array("TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]), "CACHE_GROUP" => array($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), "MULTI" => "N")), $arSectionFilter, false, array('ID', 'DESCRIPTION', 'PICTURE', 'DETAIL_PICTURE', 'IBLOCK_ID', 'UF_TOP_SEO'));
...
$itemsCnt = CNextCacheB2c::CIblockElement_GetList(array("CACHE" => array("TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]), "CACHE_GROUP" => array($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))), $arItemFilter, array());
CNextB2c::AddMeta(
	array(
...
$arSubSectionFilter = CNextB2c::GetCurrentSectionSubSectionFilter($arResult["VARIABLES"], $arParams, $arSection['ID']);
$arSubSections = CNextCacheB2c::CIblockSection_GetList(array("CACHE" => array("TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]), "MULTI" => "Y")), $arSubSectionFilter, false, array("ID", "DEPTH_LEVEL"));
...
<?else:?>
	...
	<?//seo
	$arParams["LANDING_IBLOCK_ID"] = (!isset($arParams["LANDING_IBLOCK_ID"]) || !$arParams["LANDING_IBLOCK_ID"] ? CNextCacheB2c::$arIBlocks[SITE_ID]["aspro_next_catalog"]["aspro_next_catalog_info"][0] : $arParams["LANDING_IBLOCK_ID"]);
	$arParams["TIZERS_IBLOCK_ID"] = (!isset($arParams["TIZERS_IBLOCK_ID"]) || !$arParams["TIZERS_IBLOCK_ID"] ? CNextCacheB2c::$arIBlocks[SITE_ID]["aspro_next_content"]["aspro_next_tizers"][0] : $arParams["TIZERS_IBLOCK_ID"]);
	$arSeoItems = CNextCacheB2c::CIBLockElement_GetList(array('CACHE' => array("MULTI" =>"Y", "TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["LANDING_IBLOCK_ID"]))), array("IBLOCK_ID" => $arParams["LANDING_IBLOCK_ID"], "ACTIVE"=>"Y"), false, false, array("ID", "IBLOCK_ID", "NAME", "PREVIEW_TEXT", "DETAIL_PICTURE", "PROPERTY_FILTER_URL", "PROPERTY_FORM_QUESTION", "PROPERTY_TIZERS", "PROPERTY_SECTION", "PROPERTY_LINK_REGION", "PROPERTY_SECTION_SERVICES", "DETAIL_TEXT", "PROPERTY_SEO_TEXT", "ElementValues"));
	$arSeoItem = array();
	if($arSeoItems)
	{
		$iLandingItemID = 0;
