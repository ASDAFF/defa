...
<?if(\Bitrix\Main\Loader::includeModule('aspro.next.b2c'))
{
	...
	if(!$arResult['POPUP']  || ($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.next.b2c', 'REGIONALITY_SEARCH_ROW', 'N') != 'Y'))
	{
		$arResult['REGIONS'] = CNextRegionalityB2c::getRegions();
		$arResult['CURRENT_REGION'] = CNextRegionalityB2c::getCurrentRegion();
		$arResult['REAL_REGION'] = CNextRegionalityB2c::getRealRegionByIP();
	...
	if($arResult['REGIONS'])
	{
		...
		if($arResult['POPUP'])
		{
			...
			if($arSectionsID)
			{
				$arSections = CNextCacheB2c::CIBlockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('GROUP' => 'ID', 'TAG' => CNextCacheB2c::GetIBlockCacheTag(CNextCacheB2c::$arIBlocks[SITE_ID]['aspro_next_regionality']['aspro_next_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', 'ID' => $arSectionsID, 'IBLOCK_ID' => CNextCacheB2c::$arIBlocks[SITE_ID]['aspro_next_regionality']['aspro_next_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'LEFT_MARGIN', 'RIGHT_MARGIN', 'DEPTH_LEVEL'));
				if($arSections)
				{
					$arSections2 = array();
					foreach($arSections as $key => $arSection)
					{
						...
						if($arSection['DEPTH_LEVEL'] > 1)
						{
							...
							$arTmpSection = CNextCacheB2c::CIBlockSection_GetList(array('CACHE' => array('MULTI' => 'N', 'TAG' => CNextCacheB2c::GetIBlockCacheTag(CNextCacheB2c::$arIBlocks[SITE_ID]['aspro_next_regionality']['aspro_next_regions'][0]))), array('GLOBAL_ACTIVE' => 'Y', '<=LEFT_BORDER' => $arSection['LEFT_MARGIN'], '>=RIGHT_BORDER' => $arSection['RIGHT_MARGIN'], 'DEPTH_LEVEL' => 1, 'IBLOCK_ID' => CNextCacheB2c::$arIBlocks[SITE_ID]['aspro_next_regionality']['aspro_next_regions'][0]), false, array('ID', 'IBLOCK_ID', 'NAME', 'SORT'));
	...
	elseif($arResult['POPUP'] && \Bitrix\Main\Config\Option::get('aspro.next.b2c', 'REGIONALITY_SEARCH_ROW', 'N') == 'Y')
	{
		$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH.'/js/jquery-ui.min.js');
		$this->IncludeComponentTemplate();
	}
