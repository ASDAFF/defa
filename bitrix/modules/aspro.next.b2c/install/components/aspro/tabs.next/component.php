...
if($bCatalogIndex)
{
	$arShowProp = CNextCacheB2c::CIBlockPropertyEnum_GetList(Array("sort" => "asc", "id" => "desc", "CACHE" => array("TAG" => CNextCacheB2c::GetPropertyCacheTag($arParams["TABS_CODE"]))), Array("ACTIVE" => "Y", "IBLOCK_ID" => $arParams["IBLOCK_ID"], "CODE" => $arParams["TABS_CODE"]));
	...
	if($arShowProp)
	{
		...
		foreach($arShowProp as $key => $prop)
		{
			...
			$arItems = CNextCacheB2c::CIBLockElement_GetList(array('CACHE' => array("MULTI" => "N", "TAG" => CNextCacheB2c::GetIBlockCacheTag($arParams["IBLOCK_ID"]))), array_merge($arFilter, $arrFilter, $arrFilter, $arFilterProp), false, array("nTopCount" => 1), array("ID"));
			if($arItems)
			{
				$arTabs[$key] = array(
					"TITLE" => $prop,
