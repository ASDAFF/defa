		...
		if($Auth)
		{
			if($_POST["DOMAIN"] && $_POST["TOKEN"])
			{
				if(count(explode('.', $_POST["DOMAIN"])) > 2)
					$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproNextCRMB2c::FLOWLU_PATH);
				else
					$url = str_replace("#DOMAIN#", $_POST["DOMAIN"], \Aspro\Functions\CAsproNextCRMB2c::FLOWLU_SHORT_PATH);

				$arPostFields = array(
					"api_key" => $_POST["TOKEN"]
				);
				echo \Aspro\Functions\CAsproNextCRMB2c::query($url, "/api/v1/module/crm/lead/list", $arPostFields);
			}
			else
			{
				$arError = array(
			...
			foreach($arTabs as $key => $arTab)
			{
				...
				if($domain && $token)
				{
					if(count(explode('.', $domain)) > 2)
						$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproNextCRMB2c::FLOWLU_PATH);
					else
						$url = str_replace("#DOMAIN#", $domain, \Aspro\Functions\CAsproNextCRMB2c::FLOWLU_SHORT_PATH);
					
					$arPostFields = array(
						"api_key" => $token
					);
					$result = json_decode(\Aspro\Functions\CAsproNextCRMB2c::query($url, "/api/v1/module/crm/lead/list", $arPostFields), true);
																				...
																				<tr>
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							<?foreach(\Aspro\Functions\CAsproNextCRMB2c::$arCrmFileds["FLOWLU"] as $key2 => $text):?>
