...
use Bitrix\Main\Loader,
	...
	Aspro\Next\SearchQueryB2c;

Loc::loadMessages(__FILE__);

$moduleID = 'aspro.next.b2c';
...
if(isset($_POST['action']) && $_POST['action'] === 'getQueryExplanation'){
	...
	if(isset($_POST['query']) && strlen($_POST['query'])){
		...
		list($query, $hash, $arData) = SearchQueryB2c::getSentenceMeta($query);
		if($hash !== SearchQueryB2c::META_HASH_NOT_VALID){
			$example = SearchQueryB2c::getSentenceExampleQuery($query);
			...
			if($hash & SearchQueryB2c::META_HASH_HAS_FIXED_COUNT){
				$cntFixedCount = $hash >> 16;
				$arExplanations[] = array(
					'LOGIC' => '&&',
					'TEXT' => Loc::getMessage('QUERY_EXPLANATION_CONTAINS').' <a title="'.Loc::getMessage('QUERY_EXPLANATION_CONTAINS_EQUAL').'">'.Loc::getMessage('QUERY_EXPLANATION_EQUAL').' '.SearchQueryB2c::vail($cntFixedCount, array(Loc::getMessage('QUERY_EXPLANATION_WORD1'), Loc::getMessage('QUERY_EXPLANATION_WORD2'), Loc::getMessage('QUERY_EXPLANATION_WORD0'))).'</a>',
				);
			}
			else{
				$cntAll = ($hash & (255 << 8)) >> 8;
				if($cntAll > 0){
					$arExplanations[] = array(
						'LOGIC' => '&&',
						'TEXT' => Loc::getMessage('QUERY_EXPLANATION_CONTAINS').' <a title="'.Loc::getMessage('QUERY_EXPLANATION_CONTAINS_MINIMAL').'">'.Loc::getMessage('QUERY_EXPLANATION_MINIMAL').' '.SearchQueryB2c::vail($cntAll, array(Loc::getMessage('QUERY_EXPLANATION_WORD1'), Loc::getMessage('QUERY_EXPLANATION_WORD2'), Loc::getMessage('QUERY_EXPLANATION_WORD0'))).'</a>',
					);
				}
			}

			if($hash & SearchQueryB2c::META_HASH_HAS_FIXED_FORMS && strlen($fixedForms)){
			...
			if($hash & SearchQueryB2c::META_HASH_HAS_STOP_WORDS && strlen($stopWords)){
			...
			if($hash & SearchQueryB2c::META_HASH_HAS_FIXED_ORDER && strlen($fixedOrder)){
			...
			if($hash & SearchQueryB2c::META_HASH_HAS_COMPLEX && $arComplex){
			...
			if($hash & SearchQueryB2c::META_HASH_HAS_MINUS_WORDS && ($minusWords['WORDS'] || $minusWords['STEM'])){
