	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$Auth.$SendCrm) && $RIGHT >= "W" && check_bitrix_sessid())
	{
		...
		elseif($SendCrm)
		{
			if($_POST["SITE_ID"] && $_POST["FORM_ID"] && $_POST["RESULT_ID"])
			{
				...
				echo \Aspro\Functions\CAsproNextB2c::sendLeadCrmFromForm($_POST["FORM_ID"], $_POST["RESULT_ID"], 'AMO_CRM', $_POST["SITE_ID"], true, true);
			}
			else
			{
				$arError = array(
									...
									<tr>
										<td colspan="2">
											...
											if($bIntegrationAmoCrm)
											{
												...
												if(isset($result["response"]) && ($result["response"] && isset($result["response"]["auth"]) && $result["response"]["auth"]))
												{
													$url = str_replace('#DOMAIN#', Option::get($moduleID, 'DOMAIN_AMO_CRM', '', $optionsSiteID), \Aspro\Functions\CAsproNextCRMB2c::AMO_CRM_PATH);
													$result_text = \Aspro\Functions\CAsproNextCRMB2c::query($url, "/private/api/v2/json/accounts/current/", array(), true, true);
																...
																if($codeGroup != "customers")
																{
																	foreach($arGroup as $arProp)
																	{
																		...
																		if(isset($arProp["enums"]))
																		{
																			...
																			{
																				\Aspro\Functions\CAsproNextCRMB2c::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$keyEnum."_".$codeGroup] = (Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) ? Loc::getMessage("AMO_CRM_FIELD_".$arProp["code"]."_".$enum) : iconv("UTF-8", LANG_CHARSET, $arProp["name"]."_".$enum));
																			}
																		}
																		else
																		{
																			\Aspro\Functions\CAsproNextCRMB2c::$arCrmFileds["AMO_CRM"][$codeGroup]["PROPS"][$arProp["id"]."_".$codeGroup] = iconv("UTF-8", LANG_CHARSET, $arProp["name"]);
																		}
																	}
																}
															}
																				...
																				<tr>
																					<td class="adm-detail-content-cell-l" style="width:50%;">
																						<select name="CRM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_crm" style="width:300px;">
																							...
																							<?foreach(\Aspro\Functions\CAsproNextCRMB2c::$arCrmFileds["AMO_CRM"] as $groupCode => $arGroup):?>
																					...
																					<td style="width:50%;">
																						<select name="CRM_FORM_FIELD_<?=$optionsSiteID?>[<?=$key;?>][]" class="field_form" style="width:300px;">
																							<option value=""><?=Loc::getMessage('FORM_FIELD_CRM_FIELDS_FORM_NAME_NO')?></option>
																							<optgroup label="...">
																								<?foreach(\Aspro\Functions\CAsproNextCRMB2c::$arCrmFileds["MAIN"] as $key2 => $text):?>
