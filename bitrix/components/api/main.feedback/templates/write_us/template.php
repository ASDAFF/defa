...
<div class="api-feedback <?=$tpl_class_name;?>" style="<?= $arParams['FORM_STYLE'] ?>">
	...
	if(!empty($arResult["OK_MESSAGE"]))
	{
        ...
        if($_REQUEST["anc"] == 'requestacall' )	{ 
                $eventAction = 'form_callback';
        } else if($_REQUEST["anc"] == 'rukovoditel' || $_REQUEST["anc"] == 'review') {
                $eventAction = 'form_write_us';
        } else if($_REQUEST["anc"] == 'manager') { 
                $eventAction = 'departure_manager';
        }
        $eventAction;
        ?>
        <form
                ...
                onsubmit="location.href+='<?='#'.$arParams['ANCHOR']?>';
dataLayer.push({'event': 'feedback', 'EventCategory': 'send', 'EventAction': '<?= $eventAction; ?>'});
yaCounter<?= $_SESSION['yandex_counter']; ?>.reachGoal('feedback');">		
		<?= bitrix_sessid_post() ?>
...
<!--        <span style='display: block; padding: 20px 0;'>--><?//=$arParams['FORM_DESC']?><!--</span>-->
		...
		<? endif; ?>
		<? if($arParams["ITERATION"] == "Y") : ?>
			<input type="hidden" name="LAST_ITERATION" value="Y">
		<? endif;?>
			...
			foreach($arParams['DISPLAY_FIELDS'] as $FIELD)
			{
				...
				<div class="<?=$tpl_class_name;?>_<?= ToLower($FIELD) ?>" style="<?//= $arParams['FORM_STYLE_DIV'] ?>">
					...
					<?if ( $FIELD != 'AUTHOR_CITY' || $arParams['AUTHOR_CITY_IS_LIST'] != true) {
						...
						?>
						<input id="<?=$tpl_class_name;?>_<?= ToLower($FIELD) ?>" style="<?//= $arParams['FORM_STYLE_INPUT'] ?>" type="text" value="<?= $_REQUEST[ToLower($FIELD)] ? $_REQUEST[ToLower($FIELD)] : $arResult[$FIELD]?>"
							placeholder="Введите <?=mb_strtolower($label, 'UTF-8')?>" name="<?= ToLower($FIELD) ?>"
						...
						<label for="<?= $tpl_class_name; ?>_<?= ToLower($FIELD) ?>">
							<?=$label?>
						</label>
					<?} else {?>
						<label class="author_city_list" for="author_city_list"><?=GetMessage("MFT_AUTHOR_CITY")?></label>
						<div class="author_city_list__wrap_input">
							<select name="<?=ToLower($FIELD)?>" onchange="this.style.color='#000';" id="author_city_list">
								<option value="" selected="selected" disabled><?=$field_name.((empty($arParams["REQUIRED_FIELDS"]) || in_array($FIELD, $arParams["REQUIRED_FIELDS"]))? ' *': '')?></option>
								<?foreach ( $arParams['AUTHOR_CITY_LIST'] as $k=>$v) {?>
									<option value="<?=$k?>" <?= $k == $_REQUEST[ToLower($FIELD)] ? 'selected="selected"' : ($k == $_SESSION['city_name']) ? 'selected="selected"' : '' ;?> ><?=$k?></option>
								<?}?>
							</select>
						</div>
				...
				switch($arFields[1])
				{
					case "select":
						...
						<div class="<?=$tpl_class_name;?>_<?= ToLower('CUSTOM_FIELD_' . $fk) ?>" style="<?= $arParams['FORM_STYLE_DIV'] ?>">
							<?if(!$arParams['HIDE_FIELD_NAME']):?>
							<label style="<?= $arParams['FORM_STYLE_LABEL'] ?>" for="<?=$tpl_class_name;?>_<?= ToLower('CUSTOM_FIELD_' . $fk) ?>">
					...
					case "textarea":
						...
						<div class="<?=$tpl_class_name;?>_<?= ToLower('CUSTOM_FIELD_' . $fk) ?>" style="<?= $arParams['FORM_STYLE_DIV'] ?>">
							...
							<textarea name="CUSTOM_FIELDS[<?=$fk?>][]"
									  ...
									  <?if($arParams['INCLUDE_PLACEHOLDER']):?> placeholder="<?= $arFields[0] ?>" <?endif?>
									  <? if(in_array("required", $arFields)): ?> class="required"<? endif ?>><?= $_REQUEST[ToLower($FIELD)] ? $_REQUEST[ToLower($FIELD)] : $arResult["CUSTOM_FIELD_".$fk]?></textarea>
						</div>
						<?
					break;
					//\\v1.2.9
			...
			foreach($arParams['DISPLAY_FIELDS'] as $FIELD)
			{
				...
				<div class="<?=$tpl_class_name;?>_<?= ToLower($FIELD) ?>" style="<?//= $arParams['FORM_STYLE_DIV'] ?>">
					...
					<textarea style="<?//=$arParams['FORM_STYLE_TEXTAREA'] ?>" <?if($arParams['INCLUDE_PLACEHOLDER']):?>
						placeholder="<?= $_REQUEST["anc"] == 'consultation' ? 'Ваш вопрос' : 'Комментарий'?>" <?endif?>
						<?/*placeholder="<?=$field_name.((empty($arParams["REQUIRED_FIELDS"]) || in_array($FIELD, $arParams["REQUIRED_FIELDS"]))? ' *': '')?>" <?endif*/?>
					  name="<?= ToLower($FIELD) ?>"
						<? if(empty($arParams["REQUIRED_FIELDS"]) || in_array($FIELD, $arParams["REQUIRED_FIELDS"])): ?> class="required"<? endif ?>><?= $_REQUEST[ToLower($FIELD)] ? $_REQUEST[ToLower($FIELD)] : '' ?></textarea>
				</div>
				<?
				}
			}
		...
		<? endif; ?>
        <div class="agreement" style="text-align: left; padding: 8px 0 8px; font-size: 12px; color: rgba(51, 51, 51, 0.6);">
            Нажимая на кнопку, вы подтверждаете свое согласие на обработку персональных данных в соответствии с <a style="color: #cf1f26; text-decoration: underline;" target="_blank" href="/politika-konfidentsialnosti/">Политикой конфиденциальности</a>
        </div>
		<div class="api-submit" style='text-align: center;'>
<!--			<span style="display: block; padding: 0 0 12px; text-align: left;">--><?//echo GetMessage("REQUIRE_FIELDS_NOTE");?><!--</span>-->
...
<?
$prevStep = '<a href="javascript:void(0);" class="prev_step">' . file_get_contents ( $_SERVER['DOCUMENT_ROOT'].'/images/arrow_top.svg' ). '</a>';
echo $prevStep;?>
...
<script type="text/javascript">
	$(document).ready(function(){
		$('.prev_step').appendTo('.fancybox-skin');
		$('.prev_step').click(function () {
			$data = $('form[name=api_feedback_form] input:not(:hidden), form[name=api_feedback_form] select, form[name=api_feedback_form] textarea').serialize();
			console.log($data);
			...
			$.fancybox({
				...
				'titleShow': false,
				'type': 'ajax',
				'href': '/ajax_popup.php?anc=write_us&from=<?= $_REQUEST['anc']?>&'+ $data,
			});
		});
