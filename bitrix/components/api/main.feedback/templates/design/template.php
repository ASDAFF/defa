...
?>
<style>
    .defo_feedback form input[type=text]::-webkit-input-placeholder,
    .defo_feedback form select::-webkit-input-placeholder {
        color: rgba(51, 51, 51, 0.6);
        opacity: 1;
        transition: all 0.4s;
    }
    .defo_feedback form input[type=text]::-moz-placeholder,
    .defo_feedback form select::-moz-placeholder {
        color: rgba(51, 51, 51, 0.6);
        opacity: 1;
        transition: all 0.4s;
    }
    ...
    .defo_feedback form input[type=text]:-ms-input-placeholder,
    .defo_feedback form select:-ms-input-placeholder {
        color: rgba(51, 51, 51, 0.6);
        opacity: 1;
        transition: all 0.4s;
    }
    .defo_feedback form input[type="text"], .defo_feedback form select, .defo_feedback form textarea{
        min-width: auto;
    }
    .defo_feedback form .wrapper_inputs > div{
        padding: 18px 0 8px;
    }
    .defo_feedback span{
        font-size: 16px;
...
<div class="api-feedback <?=$tpl_class_name;?>" style="<?= $arParams['FORM_STYLE'] ?>">
	...
	<form
		...
		onsubmit="location.href+='<?='#'.$arParams['ANCHO']?>'; ga('send', 'event', 'form', 'submit', 'feedback');yaCounter<?= $_SESSION['yandex_counter']; ?>.reachGoal('feedback'); <?= $arParams['ECOMMERCE_GOAL'] ? $arParams['ECOMMERCE_GOAL'] : ''?>">
	...
	<!--        <span style='display: block; padding: 20px 0;'>--><?//=$arParams['FORM_DESC']?><!--</span>-->
							...
							<div class="author_city_list__wrap_input">
								<select name="<?=ToLower($FIELD)?>" onchange="this.style.color='#000';" id="author_city_list">
									...
									<?foreach ( $arParams['AUTHOR_CITY_LIST'] as $k=>$v) {?>
										<option value="<?=$k?>" <?= ($k == $_SESSION['city_name']) ? 'selected="selected"' : '' ;?> ><?=$k?></option>
									<?}?>
								</select>
							</div>
			...
			<div style="display: table">
				<div class="tpl_default_author_square" style="display: table-cell; padding-right: 8px;">
					<input id="tpl_default_author_square" style="" value="<?=$arResult["AUTHOR_SQUARE"]?>" placeholder="Площадь" name="author_square" type="text">
				</div>

				 <div class="tpl_default_author_places" style="display: table-cell; padding-left: 8px;">
					<input id="tpl_default_author_places" style="" value="<?=$arResult["AUTHOR_PLACES"]?>" placeholder="Рабочих мест" name="author_places" type="text">
				 </div>
			</div>
			...
			<div class="agreement" style="text-align: left; padding: 8px 0 8px; font-size: 12px; color: rgba(51, 51, 51, 0.6);">
				* - 2D-модель офиса, до 5 рабочих мест<br>
				- после уточнения позиций мебели с менеджером<br>
				- если у Вас готов план помещения<br><br>
