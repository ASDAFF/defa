...
if ($bCatalogIndex) {
    if ($arParams['TABS_CODE'] != 'HIT') {
        $arPodFilter = [];
        $arEnumFilter = Array("ACTIVE" => "Y", "IBLOCK_ID" => $arParams["IBLOCK_ID"], "CODE" => $arParams["TABS_CODE"]);
        if (!empty($GLOBALS['arrFilterPodborki'])) {
            $arPodFilter = $GLOBALS['arrFilterPodborki'];
        }
        $arEnumFilter = array_merge($arEnumFilter, $arPodFilter);
        $arShowProp = CNextCache::CIBlockPropertyEnum_GetList(Array(
            "sort" => "asc",
            "id" => "desc",
            "CACHE" => array("TAG" => CNextCache::GetPropertyCacheTag($arParams["TABS_CODE"]))
        ), $arEnumFilter);
        ...
        if ($arShowProp) {
            ...
            foreach ($arShowProp as $key => $prop) {
                ...
                $arItems = CNextCache::CIBLockElement_GetList(array(
                    'CACHE' => array(
                        "MULTI" => "N",
                        "TAG" => CNextCache::GetIBlockCacheTag($arParams["IBLOCK_ID"])
                    )
                ), array_merge($arFilter, $arrFilter, $arrFilter, $arFilterProp), false, array("nTopCount" => 1),
                    array("ID"));
                if ($arItems) {
                    $arTabs[$key] = array(
                        "CODE" => $key,
