...
//iblock price
         ...
         * YANDEX PROMOS
         */
        if ($arParams['YM_PROMO_GIFT'] == 'Y') {
            $res = CIBlockElement::Getlist([],
                ['IBLOCK_ID'=>$arParams['YM_PROMO_GIFT_IBLOCK'],'ACTIVE'=>'Y','ACTIVE_DATE'=>'Y', '!DATE_ACTIVE_FROM'=>false,'!DATE_ACTIVE_TO'=>false],
                false, false,
                ['ID','NAME','PREVIEW_PICTURE']);
            $gifts = [];
            $giftIDs = [];
            while ($gift = $res->GetNext()) {
                $gift['PICTURE'] = CFile::GetPath($gift['PREVIEW_PICTURE']);
                $gifts[$gift['ID']] = $gift;
                $giftIDs[] = $gift['ID'];
            }
            $arResult['PROMO_GIFTS'] = $gifts;
        }
        if ($arParams['YM_PROMO_FLASH_DISCOUNT'] == 'Y' || $arParams['YM_PROMO_GIFT'] == 'Y') {
            // Get actions from iblock
            // Offers have link to action
            // Actions have links to gifts
            // Actions have links to offer-gifts
            $promos = [];
            $promoIDs = [];
            $res = CIBlockElement::Getlist([],
                ['IBLOCK_ID'=>$arParams['YM_PROMO_IBLOCK'],'ACTIVE'=>'Y','ACTIVE_DATE'=>'Y', '!DATE_ACTIVE_FROM'=>false,'!DATE_ACTIVE_TO'=>false],
                false, false,
                ['ID','NAME','PREVIEW_TEXT','DETAIL_PAGE_URL','DATE_ACTIVE_FROM','DATE_ACTIVE_TO','PROPERTY_YM_GIFTS','PROPERTY_YM_OFFER_GIFTS']);
            while ($el = $res->GetNextElement()) {
                $promo = $el->GetFields();
                $promo['PROPERTIES'] = $el->GetProperties();
                ...
                if (!empty($promo['PROPERTIES']['YM_GIFTS']['VALUE']) || !empty($promo['PROPERTIES']['YM_OFFER_GIFTS']['VALUE'])) {
                    $promo['TYPE'] = 'GIFTS';
                } else {
                    $promo['TYPE'] = 'FLASH';
                }
            ...
            if (count($promoIDs) > 0) {
                ...
                $arSelect = ['IBLOCK_ID','ID','NAME','PROPERTY_YM_PROMO'];

                if ($arParams['YM_PROMO_FLASH_DISCOUNT'] == 'Y') {
                    $arSelect[] = $priceSelector;
                }

                $res = CIBlockElement::Getlist(
                    [], $productsFilter, false, false, $arSelect
                );
                $promoProducts = [];
