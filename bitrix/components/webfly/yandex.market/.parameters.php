...
$arComponentParameters["PARAMETERS"]["YM_PROMOS"] = array(
    "PARENT" => "YM_PROMO",
    "NAME" => GetMessage("YM_PROMOS"),
    "TYPE" => "CHECKBOX",
    "DEFAULT" => "Y",
    "REFRESH"=>"Y"
);
if ($arCurrentValues["YM_PROMOS"] == "Y") {
    $ibs = [];
    ...
    $arComponentParameters["PARAMETERS"]["YM_PROMO_GIFT"] = array(
        "PARENT" => "YM_PROMO",
        "NAME" => GetMessage("YM_PROMO_GIFT"),
        "TYPE" => "CHECKBOX",
        "DEFAULT" => "N",
        "REFRESH"=>"Y"
    );

    if ($arCurrentValues["YM_PROMO_GIFT"] == "Y") {
        $ibs = [];
        $res = CIBlock::GetList(["sort" => "asc"], ["ACTIVE" => "Y", '!TYPE'=>$arCurrentValues['IBLOCK_TYPE_LIST']]);
        while ($ib = $res->Fetch()) {
            $ibs[$ib['ID']] = '['.$ib['CODE'].'] '.$ib['NAME'];
        }
        $arComponentParameters["PARAMETERS"]["YM_PROMO_GIFT_IBLOCK"] = array(
            "PARENT" => "YM_PROMO",
            "NAME" => GetMessage("YM_PROMO_GIFT_IBLOCK"),
            "TYPE" => "LIST",
            "VALUES" => $ibs,
            "MULTIPLE" => "N",
            "DEFAULT" => "",
