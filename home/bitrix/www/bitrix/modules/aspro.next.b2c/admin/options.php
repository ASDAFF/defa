...
if($RIGHT >= "R"){
	...
	foreach($arSites as $key => $arSite){
		...
		$arTabs[] = array(
			...
			"SITE_ID" => $arSite["ID"],
			"SITE_DIR" => $arSite["DIR"],
			"TEMPLATE" => CNextB2c::GetSiteTemplate($arSite["ID"]),
	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) > 0 && $RIGHT >= "W" && check_bitrix_sessid()){
		...
		if(strlen($RestoreDefaults) > 0){
			Option::delete(CNextB2c::moduleID);
			Option::delete(CNextB2c::moduleID, array("name" => "NeedGenerateCustomTheme"));
			Option::delete(CNextB2c::moduleID, array("name" => "NeedGenerateCustomThemeBG"));
			$APPLICATION->DelGroupRight(CNextB2c::moduleID);
		}
		else{
			Option::delete(CNextB2c::moduleID, array("name" => "sid"));
	...
	<?else:?>
		...
		if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) && check_bitrix_sessid())
		{
			...
			<?endif;?>
			<?if(CNextB2c::IsCompositeEnabled()):?>
