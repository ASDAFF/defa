...
if ($_REQUEST['id']) {
    ...
    $iterator = CIblockSection::GetList([], ['IBLOCK_ID' => $IBLOCK_ID, '=ID' => $_REQUEST['id']], false, ['ID', 'NAME', 'PICTURE', 'UF_DISCOUNT', 'UF_METKA', 'DESCRIPTION', 'SECTION_PAGE_URL', "UF_MAIN_PHOTO", "UF_MORE_PHOTO", "UF_TIZERS", "UF_PREVIEW", "UF_SERIES_GALLERY", "UF_SERIES_BTN_NAME", "UF_SERIES_BTN_FORM"]);
    if ($section = $iterator->GetNext()) {
        ...
        if (!empty($productsIds)) {
            ...
            usort($arResult["COLORS"], function ($a, $b) {
                return ($a['COUNT'] < $b['COUNT']);
            });
            usort($arResult["COLORS_ADD"], function ($a, $b) {
                return ($a['COUNT'] < $b['COUNT']);
            });
            ...
            if ($section['UF_DISCOUNT']) {
                ...
                while ($el = $res->fetch()) {
                    $seriesGalleries[] = $arFileTmp = CFile::ResizeImageGet(
                        $el['PROPERTY_PICTURES_VALUE'],
                        array("width" => 750, "height" => 350),
        ...
        <div class="row">
            ...
            <div class="col-sm-6">
                        ...
                        <div class="series-item-color-content">
                            ...
                            foreach ($arResult['COLORS'] as $color) { ?>
                                <div class="series-item-color-wrapper">
                                    <div class="series-item-color-pic" data-title="<?= $color['UF_NAME'] ?>"
                                         style="background: url(<?= $color['FILE_SRC'] ?>)">
                                        <a href="#" class="series-item-color-link series-item-color-link-main"
                                           data-color-xml-id="<?= $color['UF_XML_ID'] ?>"></a>
                                    </div>
                                </div>
                            <?
                            } ?>
                        ...
                        <div class="series-item-color-content">
                            ...
                            foreach ($arResult['COLORS_ADD'] as $color) { ?>
                                <div class="series-item-color-wrapper">
                                    <div class="series-item-color-pic" data-title="<?= $color['UF_NAME'] ?>"
                                         style="background: url(<?= $color['FILE_SRC'] ?>)">
                                        <a href="#" class="series-item-color-link series-item-color-link-add"
                                           data-color-xml-id="<?= $color['UF_XML_ID'] ?>"></a>
                                    </div>
                                </div>
                            <?
                    ...
                    <div class="series-item-color series-item-color__modal">
                        ...
                        <div class="another-color">
                            ...
                            <a class="btn"><span class="animate-load" data-event="jqm"
                                                 data-param-form_id="SIMPLE_FORM_11" data-name="SIMPLE_FORM_11"
                ...
                <div class="buttons">
                    <div class="col-sm-6">
                            <span class="btn" data-event="jqm" data-param-form_id="<?= $rsForm["SID"] ?>"
                                  data-name="<?= $rsForm["SID"] ?>"
                                  data-autoload-product_name="<?= $section["NAME"]; ?>"
                                  data-autoload-product_id="<?= $section["ID"]; ?>">
