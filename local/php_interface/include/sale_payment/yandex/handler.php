...
class YandexHandler extends PaySystem\ServiceHandler implements PaySystem\IRefundExtended, PaySystem\IHold
{
	...
	{
		$shopId = $this->getLocationShopId($payment);
...
//			"KV" => Loc::getMessage("SALE_HPS_YANDEX_TinkoffBank"),
	...
	public function getLocationShopId(Payment $payment){

		$collection = $payment->getCollection();
		$order = $collection->getOrder();
		$clientOrderId = $payment->getOrderId();
		$propertyCollection = $order->getPropertyCollection();
		$somePropValue = $propertyCollection->getDeliveryLocation();
		$delivery_location = $somePropValue->getValue();

		$shopId = getInitLocationShopId(IntVal($delivery_location));
